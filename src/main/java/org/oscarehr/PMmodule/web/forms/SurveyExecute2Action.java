...
public class SurveyExecute2Action extends ActionSupport {
    ...
    protected void postMessage(HttpServletRequest request, String key, String val) {
        String message = getText(key, val); // 从资源文件获取消息文本
        addActionMessage(message);
    }

    protected void postMessage(HttpServletRequest request, String key) {
        String message = getText(key); // 从资源文件获取消息文本
        addActionMessage(message);
    }
    ...
    public String survey() {
        ...
        String forwardString = survey_view();
    ...
    public String save_survey() {
        ...
        if (cancel) {
            ...
            return forwardToClientManager(String.valueOf(formBean.getClientId()));
        }
        ...
        setData(new SurveyExecuteDataBean());
        setView(new SurveyExecuteFormBean());
    ...
    public String printPreview_survey() {
        String forwardString = survey_view();
        if ("close".equals(forwardString)) {
            return "close";
        } else if ("clientManager".equals(forwardString)) {
    ...
    public String printPreview_refresh() {
        refresh_survey();
        return "printPreview";
    }
