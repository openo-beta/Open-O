...
public class PHRUserManagement2Action extends ActionSupport {
    ...
    public PHRUserManagement2Action() {
    }
    ...
    public String setRegistrationLetterData() {
    ...
    public String registerUser() {
        ...
        StringBuilder sb = new StringBuilder("/phr/indivo/RegisterIndivo.jsp?demographicNo=" + demographicNo);
        ...
        if (myOscarLoggedInInfo == null || !myOscarLoggedInInfo.isLoggedIn()) {
            sb.append("?failmessage=").append("Permission Denied: You must be logged into PHR to register users");
            try {
        ...
        sb.append("?username=").append(request.getParameter("username"));
        sb.append("?password=").append(request.getParameter("password"));
        sb.append("?email=").append(request.getParameter("email"));
        ...
        try {
            ...
            log.debug("error", e);
            sb.append("?failmessage").append("You don't have permissions to perform this action." + e.getClass().getName() + " - " + e.getMessage());
            try {
                response.sendRedirect(sb.toString());
            ...
            sb.append("?failmessage=").append("Failed creating user. User already exists.");
            sb.append("?nonUnique=").append("true");
            try {
                response.sendRedirect(sb.toString());
            } catch (IOException ex) {
                throw new RuntimeException(ex);
            ...
            if (se.getMessage().contains("EntityExistsException")) {
                sb.append("?failmessage=").append("Failed creating user. Possibly user already exists. " + se.getMessage());
                sb.append("?nonUnique=").append("true");
            } else {
                sb.append("?failmessage=").append("Failed creating user.");
            }
            try {
                response.sendRedirect(sb.toString());
            } catch (IOException ex) {
            ...
            log.debug("error", e);
            sb.append("?failmessage=").append("Failed creating PHR user." + e.getClass().getName() + " - " + e.getMessage());
            try {
                response.sendRedirect(sb.toString());
            } catch (IOException ex) {
                throw new RuntimeException(ex);
        ...
        try {
            ...
            sb.append("?DocId=").append(document.getId());
        } catch (PersistenceException pe) {
            log.error("Failed to persist registration letter", pe);
            sb.append("?failmessage=").append("Failed saving registration letter:" + pe.getMessage());
            try {
                response.sendRedirect(sb.toString());
            } catch (IOException ex) {
                throw new RuntimeException(ex);
            ...
            sb.append("?failmessage=").append("Failed persisting PHR user." + e.getClass().getName() + " - " + e.getMessage());
            try {
                response.sendRedirect(sb.toString());
            } catch (IOException ex) {
                throw new RuntimeException(ex);
        ...
        try {
            ...
            log.debug("error", e);
            sb.append("?failmessage=").append("Error adding patient relationships." + e.getClass().getName() + " - " + e.getMessage());
            try {
                response.sendRedirect(sb.toString());
            } catch (IOException ex) {
                throw new RuntimeException(ex);
    ...
    public String addPatientRelationship() {
        if (log.isDebugEnabled()) {
            WebUtils.dumpParameters(request);
        }
