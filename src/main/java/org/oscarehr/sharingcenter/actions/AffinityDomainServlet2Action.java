...
public class AffinityDomainServlet2Action extends ActionSupport {
    ...
    public String execute() throws Exception {
        ...
        String domainId = request.getParameter("configid");
        ...
        if (domainId != null && !domainId.equals("")) {
            if (action != null && !action.equals("")) {
                if (action.equals("delete")) {
                    ...
                    try {
                        affDao.remove(Integer.parseInt(domainId));
                        response.sendRedirect("manage.jsp?deleteSuccess");
                        return null;
                    } catch (NumberFormatException ex) {
                        MiscUtils.getLogger().debug("Error while parsing the string to an Integer", ex);
                    }
                }
            }
        }
        response.sendRedirect("manage.jsp?deleteFail");
        return null;
    }
