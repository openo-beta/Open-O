...
public class DrugProductDaoImpl extends AbstractDaoImpl<DrugProduct> implements DrugProductDao {
    ...
    public List<DrugProduct> findAvailableByCode(String code) {
        Query query = entityManager
                .createQuery("SELECT x FROM DrugProduct x where x.code = ?1  and x.dispensingEvent is null");
    ...
    public List<Object[]> findAllAvailableUnique() {
        Query query = entityManager.createQuery(
                "SELECT distinct x.code, x.name FROM DrugProduct x  where x.dispensingEvent is null order by x.name");
    ...
    public int getAvailableCount(String lotNumber, Date expiryDate, int amount) {
        Query query = entityManager.createQuery(
                "SELECT count(*)  FROM DrugProduct x  where x.dispensingEvent is null and x.lotNumber = ?1 and x.expiryDate = ?2 and x.amount = ?3");
    ...
    public List<DrugProduct> getAvailableDrugProducts(String lotNumber, Date expiryDate, int amount) {
        Query query = entityManager.createQuery(
                "SELECT x  FROM DrugProduct x  where x.dispensingEvent is null and x.lotNumber = ?1 and x.expiryDate = ?2 and x.amount = ?3");
        query.setParameter(1, lotNumber);
        query.setParameter(2, expiryDate);
        query.setParameter(3, amount);
    ...
    public List<LotBean> findDistinctLotsAvailableByCode(String code) {
        ...
        Query query = entityManager.createQuery(
                "SELECT distinct x.lotNumber, x.expiryDate, x.amount  FROM DrugProduct x  where x.dispensingEvent is null and x.code = ?1 order by x.lotNumber,x.expiryDate,x.amount");
        query.setParameter(1, code);

        @SuppressWarnings("unchecked")
        List<Object[]> tmp = query.getResultList();
        for (Object[] result : tmp) {
            results.add(new LotBean((String) result[0], (Date) result[1], (Integer) result[2],
                    getAvailableCount((String) result[0], (Date) result[1], (Integer) result[2])));
        }
        return results;
    }
