...
import ca.openosp.openo.commn.dao.CdsFormOptionDao;
...
import ca.openosp.openo.utility.AccumulatorMap;
...
public final class Cds4ReportUIBean {
    ...
    private SingleMultiAdmissions getAdmissionsSortedSingleMulti() {
        ...
        for (CdsClientForm form : cdsForms) {
            ...
            if (admission == null) {
                ...
                // check if the cds form is signed by a providers we're reporting on.
                if (providerIdsToReportOn != null) // if we've been asked to filter
                {
                    if (!providerIdsToReportOn.contains(form.getProviderNo())) // if the providers isn't in our allowed list
                    {
                        continue;
                    }
                }
    ...
    private HashMap<Integer, Admission> getAdmissionMap() {
        ...
        for (Program program : programs) {
            ...
            logger.debug("corresponding cds admissions count (before providers filter) :" + admissions.size());
            ...
            for (Admission admission : admissions) {
                if (isAdmissionForSelectedProviders(admission)) {
                    ...
                    if (!form.getProviderNo().equals(admission.getProviderNo())) {
                        //we know the providers who did the latest one isn't the one that did the admission, and this gets missed in the report.
                        //lets add the admission in this case of a staff change...if the providers is in our list
            ...
            //go through each admission, if latest version is a different providers (in our providers select ist, we want to add the admission to the map)
        }
    ...
    public int[] getDataRow(CdsFormOption cdsFormOption) {
        ...
        else if (cdsFormOption.getCdsDataCategory().startsWith("008-"))
            return (getGenericLatestAnswerDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("009-")) return (get009DataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("010-"))
            return (getGenericLatestAnswerDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("10a-"))
            return (getGenericLatestAnswerDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("10b-"))
            return (getGenericLatestAnswerDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("011-"))
            return (getGenericLatestAnswerDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("012-"))
            return (getGenericLatestAnswerDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("013-"))
            return (getGenericAllAnswersDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("014-"))
            return (getGenericLatestAnswerDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("015-"))
            return (getGenericLatestAnswerDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("016-"))
            return (getGenericLatestAnswerDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("16a-"))
            return (getGenericAllAnswersDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("017-"))
            return (getGenericAllAnswersDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("018-"))
            return (getGenericAllAnswersDataLineForNewAdmissionsDuringReportingPerdiod(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("019-"))
            return (getGenericAllAnswersDataLine(cdsFormOption));
        ...
        else if (cdsFormOption.getCdsDataCategory().startsWith("022-"))
            return (getGenericLatestAnswerDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("023-"))
            return (getGenericLatestAnswerDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("024-"))
            return (getGenericLatestAnswerDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("24a-"))
            return (getGenericLatestAnswerDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("025-"))
            return (getGenericLatestAnswerDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("25a-"))
            return (getGenericLatestAnswerDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("026-"))
            return (getGenericLatestAnswerDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("027-"))
            return (getGenericLatestAnswerDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("028-"))
            return (getGenericLatestAnswerDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("029-"))
            return (getGenericLatestAnswerDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("29a-"))
            return (getGenericLatestAnswerDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("030-"))
            return (getGenericLatestAnswerDataLine(cdsFormOption));
        else if (cdsFormOption.getCdsDataCategory().startsWith("031-"))
            return (getGenericLatestAnswerDataLine(cdsFormOption));
        else {
        ...
        if (bucket != null) {
            for (CdsClientForm form : bucket) {
                ...
                if (initialContactDate != null && initialContactDate.before(endDateExclusive.getTime())) {
                    ...
                    else if (assessmentDate.after(endDateExclusive.getTime()))
        ...
        if (bucket != null) {
            for (CdsClientForm form : bucket) {
                ...
                if (assessmentDate != null && assessmentDate.before(endDateExclusive.getTime())) {
                    ...
                    else if (admission.getAdmissionDate().after(endDateExclusive.getTime()))
                        endCountDate = endDateExclusive.getTime();
    ...
    private static Integer nullSafeMinMax(MinMax minMax, Integer i1, Integer i2) {
        ...
        if (minMax == MinMax.MAX) return (Math.max(i1.intValue(), i2.intValue()));
            ...
            throw (new IllegalStateException("okay I missed something : minMax=" + minMax + ", i1=" + i1 + ", i2=" + i2));
    }
    ...
    private int getTotalDayCount(CdsHospitalisationDays cdsHospitalisationDays, Calendar startBoundingDate, Calendar endBoundingDate) {
        ...
        if (cdsHospitalisationDays.getDischarged() != null && cdsHospitalisationDays.getDischarged().before(startBoundingDate))
            return (0);
        ...
        if (cdsHospitalisationDays.getAdmitted().after(startBoundingDate))
            fromCal = cdsHospitalisationDays.getAdmitted();

        Calendar toCal = endBoundingDate;
        if (cdsHospitalisationDays.getDischarged() != null && cdsHospitalisationDays.getDischarged().before(endBoundingDate))
            toCal = cdsHospitalisationDays.getDischarged();
