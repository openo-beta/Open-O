...
import ca.openosp.openo.commn.dao.ClientLinkDao;
...
import ca.openosp.openo.ws.Gender;
...
public class ManageHnrClientAction {
    ...
    public static void copyHnrToLocal(LoggedInInfo loggedInInfo, Integer clientId) throws ConnectException_Exception {
        try {
            logger.debug("copyHnrToLocal currentFacility=" + loggedInInfo.getCurrentFacility().getId()
                    + ", loggedInInfo.loggedInProvider=" + loggedInInfo.getLoggedInProviderNo() + ", client="
                    + clientId);

            List<ClientLink> clientLinks = clientLinkDao.findByFacilityIdClientIdType(
                    loggedInInfo.getCurrentFacility().getId(), clientId, true, ClientLink.Type.HNR);
            ...
            if (clientLinks.size() > 0) {
                ...
                Client hnrClient = CaisiIntegratorManager.getHnrClient(loggedInInfo,
                ...
                if (hnrClient.getCity() != null)
                    demographic.setCity(hnrClient.getCity());
                if (hnrClient.getFirstName() != null)
                    demographic.setFirstName(hnrClient.getFirstName());
                if (hnrClient.getGender() != null)
                    demographic.setSex(hnrClient.getGender().name());
                if (hnrClient.getHin() != null)
                    demographic.setHin(hnrClient.getHin());
                if (hnrClient.getHinVersion() != null)
                    demographic.setVer(hnrClient.getHinVersion());
                if (hnrClient.getHinType() != null)
                    demographic.setHcType(hnrClient.getHinType());
                if (hnrClient.getHinValidStart() != null)
                    demographic.setEffDate(DateUtils.toDate(hnrClient.getHinValidStart()));
                if (hnrClient.getHinValidEnd() != null)
                    demographic.setHcRenewDate(DateUtils.toDate(hnrClient.getHinValidEnd()));
                ...
                if (hnrClient.getLastName() != null)
                    demographic.setLastName(hnrClient.getLastName());
                if (hnrClient.getProvince() != null)
                    demographic.setProvince(hnrClient.getProvince());
                if (hnrClient.getStreetAddress() != null)
                    demographic.setAddress(hnrClient.getStreetAddress());
    ...
    public static void copyLocalValidatedToHnr(LoggedInInfo loggedInInfo, Integer clientId)
            throws DuplicateHinExceptionException, InvalidHinExceptionException, ConnectException_Exception {
        try {
            logger.debug("copyLocalToHnr currentFacility=" + loggedInInfo.getCurrentFacility().getId()
                    + ", loggedInInfo.loggedInProvider=" + loggedInInfo.getLoggedInProviderNo() + ", client="
                    + clientId);

            // there's 2 cases here
            // 1) there's a linked client at which point update the linked client on the hnr
            // 2) there is no linked client at which point create a new linked client on the
            // hnr and create the link.

            // were ignoring the anomalie of multiple hnr links as it should never really
            // happen though it's theoretically possible due to lack of atomic updates on
            // this table.
            List<ClientLink> clientLinks = clientLinkDao.findByFacilityIdClientIdType(
                    loggedInInfo.getCurrentFacility().getId(), clientId, true, ClientLink.Type.HNR);

            Client hnrClient = null;
            ...
            // can be null if there's no existing link or if the data on the hnr has been
            // revoked of consent
            if (hnrClient == null) {
                hnrClient = new Client();
            }

            // copy any non null data to the HNR if it's validated
            // you have to check for null before setting because if it's an existing record
            // you don't want to nullify existing data
            boolean isAtLeastOneThingValidated = false;
            Demographic demographic = demographicDao.getDemographicById(clientId);

            HnrDataValidation tempValidation = hnrDataValidationDao.findMostCurrentByFacilityIdClientIdType(
                    loggedInInfo.getCurrentFacility().getId(), clientId, HnrDataValidation.Type.HC_INFO);
            boolean hcInfoValidated = (tempValidation != null
                    && tempValidation.isValidAndMatchingCrc(HnrDataValidation.getHcInfoValidationBytes(demographic)));

            if (hcInfoValidated) {
                isAtLeastOneThingValidated = true;

                if (demographic.getFirstName() != null)
                    hnrClient.setFirstName(demographic.getFirstName());
                if (demographic.getLastName() != null)
                    hnrClient.setLastName(demographic.getLastName());

                try {
                    if (demographic.getSex() != null)
                        hnrClient.setGender(Gender.valueOf(demographic.getSex().toUpperCase()));
                } catch (Exception e) {
                    // do nothing, this is on a best attempt basis. until genders are defined
                    // constants.
                }

                if (demographic.getBirthDay() != null)
                    hnrClient.setBirthDate(demographic.getBirthDay());
                if (demographic.getHin() != null)
                    hnrClient.setHin(demographic.getHin());
                if (demographic.getVer() != null)
                    hnrClient.setHinVersion(demographic.getVer());
                if (demographic.getHcType() != null)
                    hnrClient.setHinType(demographic.getHcType().toLowerCase());
                if (demographic.getEffDate() != null)
                    hnrClient.setHinValidStart(DateUtils.toCalendar(demographic.getEffDate()));
                if (demographic.getHcRenewDate() != null)
                    hnrClient.setHinValidEnd(DateUtils.toCalendar(demographic.getHcRenewDate()));
            }

            ClientImage clientImage = clientImageDAO.getClientImage(clientId);
            tempValidation = hnrDataValidationDao.findMostCurrentByFacilityIdClientIdType(
                    loggedInInfo.getCurrentFacility().getId(), clientId, HnrDataValidation.Type.PICTURE);
            boolean pictureValidated = (tempValidation != null
                    && tempValidation.isValidAndMatchingCrc(clientImage.getImage_data()));
            ...
            tempValidation = hnrDataValidationDao.findMostCurrentByFacilityIdClientIdType(
                    loggedInInfo.getCurrentFacility().getId(), clientId, HnrDataValidation.Type.OTHER);
            boolean otherValidated = (tempValidation != null && tempValidation
                    .isValidAndMatchingCrc(HnrDataValidation.getOtherInfoValidationBytes(demographic)));
            if (otherValidated) {
                isAtLeastOneThingValidated = true;

                if (demographic.getAddress() != null)
                    hnrClient.setStreetAddress(demographic.getAddress());
                if (demographic.getCity() != null)
                    hnrClient.setCity(demographic.getCity());
                if (demographic.getProvince() != null)
                    hnrClient.setProvince(demographic.getProvince());
            }
            ...
            if (isAtLeastOneThingValidated) {
                ...
                // this section will produce correct results based on the model but unexpected
                // from the user, so we're hacking it
                // List<IntegratorConsent>
                // consents=integratorConsentDao.findByFacilityAndDemographic(loggedInInfo.getCurrentFacility().getId(),
                // clientId);
                // if (consents.size()>0) {
                // // only 1 hnr setting so using the latest is fine.
                // IntegratorConsent integratorConsent=consents.get(0);
                // // was asked to remove hnr consent and just apply the general consent setting
                // to the hnr
                ...
                // }
                // new hacked consent which is actually wrong but may produced "expected"
                // results
                GetConsentTransfer remoteConsent = CaisiIntegratorManager.getConsentState(loggedInInfo,
                        loggedInInfo.getCurrentFacility(), clientId);
                ...
                // save the client
                hnrClient.setUpdatedBy("faciliy: " + loggedInInfo.getCurrentFacility().getName() + ", providers:"
                        + loggedInInfo.getLoggedInProvider().getFormattedName());
                Integer linkingId = CaisiIntegratorManager.setHnrClient(loggedInInfo, loggedInInfo.getCurrentFacility(),
                        hnrClient);
                ...
                // the code is set to "ignore" multiple hnr links so it should function fine
                // anyways.
    ...
    public static void setPictureValidation(LoggedInInfo loggedInInfo, Integer clientId, boolean valid) {
        try {
            logger.debug("setPictureValidation currentFacility=" + loggedInInfo.getCurrentFacility().getId()
                    + ", loggedInInfo.loggedInProvider=" + loggedInInfo.getLoggedInProviderNo() + ", client=" + clientId
                    + ", valid=" + valid);

            ClientImage clientImage = clientImageDAO.getClientImage(clientId);
            if (!HnrDataValidation.isImageValidated(clientImage))
                throw (new IllegalStateException(
                        "Attempt to validate an image that doesn't exist, button should have been disabled. clientId="
                                + clientId));
    ...
    public static void setHcInfoValidation(LoggedInInfo loggedInInfo, Integer clientId, boolean valid) {
        try {
            logger.debug("setHcInfoValidation currentFacility=" + loggedInInfo.getCurrentFacility().getId()
                    + ", loggedInInfo.loggedInProvider=" + loggedInInfo.getLoggedInProviderNo() + ", client=" + clientId
                    + ", valid=" + valid);

            Demographic demographic = demographicDao.getDemographicById(clientId);
            if (!HnrDataValidation.isHcInfoValidateable(demographic))
                throw (new IllegalStateException(
                        "Attempt to validate a clients hc info that is not validateable, button should have been disabled. clientId="
                                + clientId));
    ...
    public static void setOtherInfoValidation(LoggedInInfo loggedInInfo, Integer clientId, boolean valid) {
        try {
            logger.debug("setOtherInfoValidation currentFacility=" + loggedInInfo.getCurrentFacility().getId()
                    + ", loggedInInfo.loggedInProvider=" + loggedInInfo.getLoggedInProviderNo() + ", client=" + clientId
                    + ", valid=" + valid);

            Demographic demographic = demographicDao.getDemographicById(clientId);
            if (!HnrDataValidation.isOtherInfoValidateable(demographic))
                throw (new IllegalStateException(
                        "Attempt to validate a clients other info that is not validateable, button should have been disabled. clientId="
                                + clientId));
