...
public class ManageLinkedClients {
    ...
    private static void addHnrLinks(LoggedInInfo loggedInInfo, Facility facility, HashMap<String, LinkedDemographicHolder> results, Demographic demographic) throws MalformedURLException, ConnectException_Exception {
        ...
        if (currentLinks.size() > 0) {
            Client hnrClient = CaisiIntegratorManager.getHnrClient(loggedInInfo, facility, currentLinks.get(0).getRemoteLinkId());
    ...
    private static void addHnrMatches(LoggedInInfo loggedInInfo, Facility facility, HashMap<String, LinkedDemographicHolder> results, Demographic demographic) throws MalformedURLException {
        try {
            ...
            for (MatchingClientScore matchingClientScore : potentialMatches) {
                ...
                Client hnrClient = matchingClientScore.getClient();
                copyHnrClientDataToMatchingScorePlaceholder(integratorLinkedDemographicHolder, hnrClient);
            }
    ...
    private static void copyHnrClientDataToMatchingScorePlaceholder(LinkedDemographicHolder integratorLinkedDemographicHolder, Client hnrClient) {
        // copy the data to holder entry
        if (hnrClient.getBirthDate() != null)
            integratorLinkedDemographicHolder.birthDate = DateFormatUtils.ISO_DATE_FORMAT.format(hnrClient.getBirthDate());
        ...
        if (hnrClient.getImage() != null)
            integratorLinkedDemographicHolder.imageUrl = "/imageRenderingServlet?source=" + ImageRenderingServlet.Source.hnr_client.name() + "&linkingId=" + hnrClient.getLinkingId();
    }
    ...
    private static void copyDemographicTransferDataToScorePlaceholder(LoggedInInfo loggedInInfo, Facility facility, DemographicTransfer demographicTransfer, LinkedDemographicHolder integratorLinkedDemographicHolder) throws MalformedURLException {
        ...
        if (demographicTransfer.getBirthDate() != null)
            integratorLinkedDemographicHolder.birthDate = DateFormatUtils.ISO_DATE_FORMAT.format(demographicTransfer.getBirthDate());
        ...
        if (demographicTransfer.getPhoto() != null)
            integratorLinkedDemographicHolder.imageUrl = "/imageRenderingServlet?source=" + ImageRenderingServlet.Source.integrator_client.name() + "&integratorFacilityId=" + demographicTransfer.getIntegratorFacilityId() + "&caisiDemographicId=" + demographicTransfer.getCaisiDemographicId();
    }
