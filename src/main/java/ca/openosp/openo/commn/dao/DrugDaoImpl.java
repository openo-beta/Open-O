...
public class DrugDaoImpl extends AbstractDaoImpl<Drug> implements DrugDao {
    ...
    public List<Drug> findByDemographicId(Integer demographicId, Boolean archived) {
        ...
        String sqlCommand = "select x from Drug x where x.demographicId=?1 "
                + (archived == null ? "" : "and x.archived=?2 ") + " order by x.createDate DESC";
    ...
    public List<Drug> findByDemographicIdOrderByDate(Integer demographicId, Boolean archived) {
        // build sql string
        String sqlCommand = "select x from Drug x where x.demographicId=?1 "
                + (archived == null ? "" : "and x.archived=?2") + " order by x.rxDate desc, x.id desc";
    ...
    public List<Drug> findByDemographicIdOrderByPositionForExport(Integer demographicId, Boolean archived) {
        // build sql string
        String sqlCommand = "select x from Drug x where x.demographicId=?1 "
                + (archived == null ? "" : "and x.archived=?2 and x.archivedReason != ?3")
                + " order by x.position desc, x.rxDate desc, x.id desc";
    ...
    public List<Drug> findByDemographicIdOrderByPosition(Integer demographicId, Boolean archived) {
        // build sql string
        String sqlCommand = "select x from Drug x where x.demographicId=?1 "
                + (archived == null ? "" : "and x.archived=?2") + " order by x.position desc, x.rxDate desc, x.id desc";
        // set parameters
        Query query = entityManager.createQuery(sqlCommand);
        query.setParameter(1, demographicId);
        if (archived != null) {
    ...
    public List<Drug> findByDemographicIdSimilarDrugOrderByDate(Integer demographicId, String regionalIdentifier,
        ...
        String sqlCommand = "select x from Drug x where x.demographicId=?1 and x."
                ...
                + "=?2 order by x.rxDate desc, x.id desc";
    ...
    public List<Drug> findByDemographicIdAndAtc(int demographicNo, String atc) {
        Query query = createQuery("d",
                "d.demographicId = :demoId AND d.atc = :atc ORDER BY d.position, d.rxDate DESC, d.id DESC");
    ...
    public List<Drug> findByDemographicIdAndRegion(int demographicNo, String regionalIdentifier) {
        Query query = createQuery("d",
                "d.demographicId = :demoId AND d.regionalIdentifier = :regionalIdentifier ORDER BY d.position, d.rxDate DESC, d.id DESC");
        query.setParameter("demoId", demographicNo);
    ...
    public List<Drug> findByDemographicIdAndDrugId(int demographicNo, Integer drugId) {
        Query query = createQuery("d",
                "d.demographicId = :demoId AND d.id = :drugId ORDER BY d.position, d.rxDate DESC, d.id DESC");
        query.setParameter("demoId", demographicNo);
    ...
    public int getMaxPosition(int demographicNo) {
        Query query = entityManager.createQuery(
                "SELECT MAX(d.position) FROM " + modelClass.getSimpleName() + " d WHERE d.demographicId = :id");
    ...
    /**
     * Finds drug by the specified brand name, demographic id and providers number.
     *
    ...
    /**
     * Finds drug by the specified custom drug name, demographic id and providers
     * number.
     *
