...
import static ca.openosp.openo.integration.mcedt.McedtConstants.*;
        ...
        try {
            ...
            if (toOutbox != null) {
                ...
                for (File file : toOutbox) {
                    if (new Date(file.lastModified()).after(startDate) && new Date(file.lastModified()).before(endDate))
...
//			}
    ...
    static String getTypeDescription(String typeCode, List<TypeListData> data) {
        ...
        String typeDesc = "";
        for (TypeListData typeListData : data) {
            if (typeListData.getResourceType().trim().equalsIgnoreCase(typeCode.trim())) {
                typeDesc = typeListData.getDescriptionEn();
                break;
            }
    ...
    static DetailDataCustom mapDetailData(DetailDataCustom detailDataK, DetailData detailData, List<TypeListData> data, String serviceIdSent) {
        ...
        detailDataK.setResourceType(getTypeDescription(detailData.getResourceType(), data));

        detailDataK.setResult(detailData.getResult());
        detailDataK.setStatus(detailData.getStatus());
        detailDataK.setServiceId(serviceIdSent);
        return detailDataK;
    }
