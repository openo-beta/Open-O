...
import ca.openosp.openo.integration.fhir.dstu3.interfaces.ResourceAttributeFilterInterface;
import ca.openosp.openo.integration.fhir.dstu3.manager.OscarFhirConfigurationManager;
import ca.openosp.openo.integration.fhir.dstu3.resources.ResourceAttributeFilter;
...
public abstract class AbstractOscarFhirResource<FHIR extends org.hl7.fhir.dstu3.model.BaseResource, OSCAR extends AbstractModel<?>>
    ...
    */
    ...
    public static void setFhirContext(FhirContext fhirContext) {
        ca.openosp.openo.integration.fhir.dstu3.model.AbstractOscarFhirResource.fhirContext = fhirContext;
    }
    ...
    return String.format("%s/%s", ((Resource) getFhirResource()).getResourceType(),
        getFhirResource().getId());
    }

    public String getContainedReferenceLink() {
    String resourceType = ((Resource) getFhirResource()).getResourceType().name();
    String referenceLink = String.format("%s%s", resourceType,
        getFhirResource().getId().replaceAll(resourceType, ""));
    ...
    */
    ...
    public String getFhirJSON() {
        return getFhirContext().newJsonParser().setPrettyPrint(true)
            .encodeResourceToString(getFhirResource());
    }
