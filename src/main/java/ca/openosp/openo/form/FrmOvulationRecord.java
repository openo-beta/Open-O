...
public class FrmOvulationRecord extends FrmRecord {
    public Properties getFormRecord(LoggedInInfo loggedInInfo, int demographicNo, int existingID)
        ...
        if (existingID <= 0) {
            ...
            String sql = "SELECT demographic_no, last_name, first_name, year_of_birth, month_of_birth, date_of_birth, hin, ver FROM demographic WHERE demographic_no = ?";
            ...
            String sql = "SELECT * FROM formovulation WHERE demographic_no = ? AND ID = ?";
    ...
    public int saveFormRecord(Properties props) throws SQLException {
        ...
        String sql = "SELECT * FROM formovulation WHERE demographic_no = ? AND ID = 0";
