...
public class FrmReceptionAssessmentRecord extends FrmRecord {
    public Properties getFormRecord(LoggedInInfo loggedInInfo, int demographicNo, int existingID)
        ...
        if (existingID <= 0) {
            ...
            String sql = "SELECT * FROM formreceptionassessment WHERE demographic_no = ? AND ID = ?";
    ...
    public int saveFormRecord(Properties props) throws SQLException {
        ...
        String sql = "SELECT * FROM formreceptionassessment WHERE demographic_no = ? AND ID = 0";
