...
public class FrmBCAR2012Record extends FrmRecord {
    ...
    public Properties getFormRecord(LoggedInInfo loggedInInfo, int demographicNo, int existingID) {
        ...
        if (existingID <= 0) {
            ...
            String sql = "SELECT * FROM formBCAR2012 WHERE demographic_no = ? AND ID = ?";
    ...
    public int saveFormRecord(Properties props) throws SQLException {
        ...
        String sql = "SELECT * FROM formBCAR2012 WHERE demographic_no=? AND ID=0";

        FrmRecordHelp frh = new FrmRecordHelp();
        frh.setDateFormat(_dateFormat);
        int recordId = ((frh).saveFormRecord(props, sql, demographic_no));
        return recordId;
    }
