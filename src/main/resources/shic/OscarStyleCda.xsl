...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:n1="urn:hl7-org:v3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
   ...
   <!-- Start Bootstrap Panel Component -->
      ...
      <xsl:choose>
         <xsl:when test="$address">
            ...
            <xsl:if test="string-length($address/n1:country)>0">
               		<xsl:text>,&#160;</xsl:text>
			<xsl:comment>
			<!--Bandaid temp fix for Canada showing up as "-1" whenever default Country isn't changed for patients -->
			</xsl:comment>
	       		<xsl:if test="contains(($address/n1:country),'-1')">
