        ...
        <td class="MainTableRightColumn">
            <table width="100%">
                <tr>
                    <td>
                        		<span id="buttonArea">
                        		<button style="float:right" onclick="refreshPassword();"
                                        title="This will set a new strong password for your account">Refresh Password</button>
                            <%
                            ...
                            %>
                           				<button style="float:right"
                                                onclick="window.open('<%=request.getContextPath()%>/myoscar/myoscar_page_link_action.jsp?redirectPage=/v2/external_message_settings.jsp');">PHR Message Settings</button>
                        ...
                        <table cellspacing=3 style="display:inline">
                            <tr>
                                ...
                                <td class="myoscarLoginElementAuth">
                                    ...
                                    </b> (<%=myOscarLoggedInInfo.getLoggedInPerson().getUserName()%>)
                                ...
                                <td class="myoscarLoginElementNoAuth">
                                    <div>
                                        <form action="../../phr/Login.do" name="phrLogin" method="POST"
                                            ...
                                            <br/>
                                            Keep me logged in <input type="checkbox" checked="checked"
                                                                     name="saveMyOscarPassword"/>
                                            <input type="hidden" name="forwardto"
                                                   value="<%=request.getServletPath()%>?method=<%=request.getParameter("method")%>">
                                        </form>
                                    </div>
                ...
                <%
                    if (myOscarLoggedInInfo != null && myOscarLoggedInInfo.isLoggedIn()) {
                        ...
                        if ("viewSentMessages".equals(pageMethod))
                            messages = MyOscarMessagesHelper.getSentMessages(session, startIndex);
                        else if ("viewArchivedMessages".equals(pageMethod))
                            messages = MyOscarMessagesHelper.getReceivedMessages(session, false, startIndex);
                        ...
                        for (MessageTransfer3 message : messages) {
                            ...
                            if (minimalPersonSender.getLastName() != null)
                                displayName.append(minimalPersonSender.getLastName()).append(", ");
                            if (minimalPersonSender.getFirstName() != null)
                                displayName.append(minimalPersonSender.getFirstName());
                            displayName.append(" (");
                            ...
                            if (recipientString.length() > 32)
                                recipientString = recipientString.substring(0, 32) + "...";
                ...
                <tr class="<%=recipientAttributes!=null && recipientAttributes.getFirstViewDate()!=null?"normal":"new"%>">
                    ...
                    <td bgcolor="#EEEEFF">
                        ...
                        <%
                            if (demographic != null) {
                        %>
                        </a> <a href="?<%=request.getQueryString()%>"
                                onClick="gotoEchart3(<%=demographic.getDemographicNo()%>);">E</a>
                        <%
                            }
                        %>
                    </td>
                    <td bgcolor="#EEEEFF">
                        <%
                            if (recipientAttributes != null) {
                                ...
                                if (demographic != null) {
                        %>
                        <a href="?<%=request.getQueryString()%>"
                           onClick="gotoEchart2(<%=demographic.getDemographicNo()%>,<%=message.getId()%>);">
                            <%
                                    }
                                }
                            %>
