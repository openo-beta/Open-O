		...
		<div class="modal-content" role="dialog">
			...
			<div class="modal-body">
				<form action="${pageContext.request.contextPath}/oscarReport/reportByTemplate/actions/rbtAddToGroup.do" id="templateToGroupForm">
					<div>
						<select style="width:100%;height:400px;outline:none;" name="tid" id="templateSelect" multiple>
							<c:if test="${ not empty templatesNotInGroup}">
								<c:forEach var="template" items="${ templatesNotInGroup }">
									<option value="${template.templateId}"><c:out value="${template.title}"/></option>
								</c:forEach>
							</c:if>
						</select>
