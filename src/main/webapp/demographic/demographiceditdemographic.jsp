...
<html>
	...
	%>
        ...
        <script language="JavaScript">
            ...
            var numMenus = 1;
            var encURL = "<c:out value="${ctx}"/>/oscarEncounter/IncomingEncounter.do?providerNo=<%=Encode.forJavaScript(String.valueOf(curProvider_no))%>&appointmentNo=&demographicNo=<%=Encode.forJavaScript(String.valueOf(demographic_no))%>&curProviderNo=&reason=<%=Encode.forJavaScript(String.valueOf(URLEncoder.encode(noteReason, StandardCharsets.UTF_8)))%>&encType=<%=Encode.forJavaScript(String.valueOf(URLEncoder.encode("telephone encounter with client", StandardCharsets.UTF_8)))%>&userName=<%=Encode.forJavaScript(String.valueOf(URLEncoder.encode( userfirstname+" "+userlastname, StandardCharsets.UTF_8)))%>&curDate=<%=Encode.forJavaScript(String.valueOf(dateString))%>&appointmentDate=&startTime=&status=";
            ...
            <tr>
                <td class="MainTableLeftColumn" valign="top">
                    <table border=0 cellspacing=0 width="100%" id="appt_table">
                        ...
                        <tr id="appt_hx">
                            <td><a
                                    href='<%= request.getContextPath() %>/demographic/demographiccontrol.jsp?demographic_no=<%=Encode.forUriComponent(String.valueOf(demographic.getDemographicNo()))%>&last_name=<%=Encode.forUriComponent(String.valueOf(URLEncoder.encode(demographic.getLastName(), StandardCharsets.UTF_8)))%>&first_name=<%=Encode.forUriComponent(String.valueOf(URLEncoder.encode(demographic.getFirstName(), StandardCharsets.UTF_8)))%>&orderby=appttime&displaymode=appt_history&dboperation=appt_history&limit1=0&limit2=25'><fmt:setBundle basename="oscarResources"/><fmt:message key="demographic.demographiceditdemographic.btnApptHist"/></a>
                            </td>
                        </tr>
