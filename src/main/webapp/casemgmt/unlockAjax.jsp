...
<c:if test="${ not success}">
    <img title="Minimize Display" id='quitImg<c:out value="${Note.id}"/>'
         ...
         src='<c:out value="${ctx}"/>/oscarEncounter/graphics/triangle_up.gif'/>
    <span id="txt<c:out value="${Note.id}"/>"><fmt:setBundle basename="oscarResources"/><fmt:message key="oscarEncounter.Index.msgLocked"/> <%=Encode.forHtml(String.valueOf(DateUtils.getDate(note.getUpdate_date(), dateFormat)))%>
