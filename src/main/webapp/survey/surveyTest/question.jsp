...
<script>
	function select_checkbox(name) {
		var element = document.surveyTestForm.elements['data.value(' + name + ')'];
		var hiddenElement = document.surveyTestForm.elements['data.value(checkbox_' + name + ')'];
	...
	<tR>
		<c:choose>
				...
				<c:choose>
					<c:when test="${question.type.openEnded.rows == 1}">
						...
						<td><html-el:text
							...
							size="${question.type.openEnded.cols}" />&nbsp;<c:out
							value="${question.unit}" /></td>
					</c:when>
					<c:otherwise>
						...
						<html-el:textarea
							...
							cols="${question.type.openEnded.cols}"></html-el:textarea> &nbsp;<c:out
							value="${question.unit}" /></td>
					</c:otherwise>
				</c:choose>
			</c:when>
			...
			<c:when test="${question.type.date != null}">
				...
				</script> &nbsp;<c:out value="${question.unit}" /></td>
			</c:when>
								...
								<c:when
									...
									<tr>
										<c:forEach var="answer"
											...
											<td><html-el:radio
												property="data.value(${pageNumber}_${sectionId}_${question.id})"
												value="${answer}" />&nbsp;<c:out value="${answer}" />&nbsp;<c:out
												value="${question.unit}" /></td>
										</c:forEach>
										<c:if test="${question.type.select.otherAnswer eq true}">
											<td><html-el:radio
												...
												property="data.value(${pageNumber}_${sectionId}_${question.id}_other_value)" />&nbsp;<c:out
												value="${question.unit}" /></td>
										</c:if>
									</tr>
								</c:when>
									...
									<tr>
										<c:forEach var="answer"
											...
											<html-el:checkbox
												...
												value="${answer}" />&nbsp;<c:out value="${question.unit}" />
											</td>
										</c:forEach>
