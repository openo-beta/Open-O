  ...
  <c:forEach var="patientBill" items="${patientBills}">
    ...
    <c:forEach items="${patientBill.invoiceItems}" var="invoice" >
      ...
      <tr>
        ...
        <td>
          <fmt:formatNumber type="currency" groupingUsed="true" minFractionDigits="2" value="${invoice.gst}" />
        </td>
        <td>
          <fmt:formatNumber type="currency" groupingUsed="true" minFractionDigits="2" value="${ invoice.bill_amount }" />
        </td>
        <td>
          <fmt:formatNumber type="currency" groupingUsed="true" minFractionDigits="2" value="${ invoice.amount_received }" />
        </td>
      </tr>
      ...
      <%-- Payment Details  --%>
      <tr><th id="payableTitle" colspan="8">Please Make Cheque Payable To:</th></tr>
      ...
      <tr style="border-top:#ccc 1px solid;">
        ...
        <% if(!StringUtils.isNullOrEmpty(payeeInfo)) { %>
          <div class="payeeInfo"><%=Encode.forHtml(payeeInfo)%></div>
        <% }
        ...
        %>
          ...
          <div><%=Encode.forHtml(clinic.getClinicAddress()+", "+clinic.getClinicCity()+", "+clinic.getClinicProvince()+" "+clinic.getClinicPostal())%> </div>
          <div id="clinicPhone"> Telephone: <%=vecPhones.size() >= 1 ? vecPhones.elementAt(0) : clinic.getClinicPhone()%> </div>
          <div id="clinicFax"> Fax: <%=vecFaxes.size() >= 1 ? vecFaxes.elementAt(0) : clinic.getClinicFax()%> </div>
          <% } else { %>

          <div class="payeeInfo"><%= Encode.forHtml(invoiceClinicInfo.getValue())%></div>
        ...
        <tr>
          ...
          <td style="text-align:right;">
            <fmt:formatNumber type="currency" groupingUsed="true" minFractionDigits="2" value="${  balance + gstTotal }" />
          </td>
        </tr>
    ...
    <c:set var="finalBalance" value="${ (balance + gstTotal) - payments }" scope="page" />
      ...
      <tr id="finalBalance" style="border-bottom:#ccc 1px solid;">
        ...
        <td style="text-align:right;">
          <strong>
          <fmt:formatNumber type="currency" groupingUsed="true" minFractionDigits="2" value="${ finalBalance }" />
          </strong>
        </td>
      </tr>
      ...
      <%-- Comments input  --%>
      <tr><th id="commentTitle" colspan="8">Comments:</th></tr>
      <tr>
        <td colspan="8">
