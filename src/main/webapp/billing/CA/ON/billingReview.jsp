...
<form name="form1" method="post" action="billingSave.jsp"
        ...
        if (othercode2.compareTo("") == 0 || othercode2 == null || othercode2.length() < 5) {
            ...
            if (otherdesc2.compareTo("") == 0 || otherdesc2 == null) {
                ...
                if (otherfee2.compareTo(".00") == 0) {
                    ...
                    } else if (otherdbcode2.compareTo("A001A") == 0 || otherdbcode2.compareTo("A003A") == 0 ||
        ...
        if (othercode3.compareTo("") == 0 || othercode3 == null || othercode3.length() < 5) {
            ...
            if (otherdesc3.compareTo("") == 0 || otherdesc3 == null) {
                ...
                if (otherfee3.compareTo(".00") == 0) {
                    ...
                    } else if (otherdbcode3.compareTo("A001A") == 0 || otherdbcode3.compareTo("A003A") == 0 ||
            ...
            for (Enumeration e = request.getParameterNames(); e.hasMoreElements(); ) {
                ...
                if (fee == null) {
                    ...
                    if (fee.compareTo(".00") == 0) {
                        ...
                        } else if (scode.compareTo("A001A") == 0 || scode.compareTo("A003A") == 0 ||
    ...
    <form><input type=button name=back value='Go Back and Change'
                 onClick='javascript:location.href="billingOB.jsp?billForm=<%=Encode.forJavaScript(request.getParameter("billForm"))%>&hotclick=<%=Encode.forJavaScript(String.valueOf(""))%>&appointment_no=<%=Encode.forJavaScript(request.getParameter("appointment_no"))%>&demographic_name=<%=Encode.forJavaScript(String.valueOf(demoname))%>&demographic_no=<%=Encode.forJavaScript(request.getParameter("demographic_no"))%>&user_no=<%=Encode.forJavaScript(request.getParameter("user_no"))%>&providerview=<%=Encode.forJavaScript(request.getParameter("apptProvider_no"))%>&apptProvider_no=<%=Encode.forJavaScript(request.getParameter("apptProvider_no"))%>&appointment_date=<%=Encode.forJavaScript(request.getParameter("xml_appointment_date"))%>&status=<%=Encode.forJavaScript(request.getParameter("status"))%>&start_time=<%=Encode.forJavaScript(request.getParameter("start_time"))%>&bNewForm=0"'>
    </form>
    ...
    function addToDiseaseRegistry() {
        if (validateItems()) {
            ...
            new Ajax.Updater('dxListing', url, {
                ...
                postBody: data,
                asynchronous: true,
                onComplete: getNewCurrentDxCodeList
            });
        } else {
