...
function editControlContents(editorname) {
    ...
    if (document.designMode) {
        ...
        if (isIE()) {
            value = window[editorname].document.body.innerHTML;
        } else {
            value = document.getElementById(editorname).contentWindow.document.body.innerHTML;
        }
    } else {
...
function Select(selectname) {
    ...
    if (cursel != 0) { // First one is a label
        ...
        if (isIE()) {
            window[cfg_editorname].document.execCommand(selectname, false, selected);
        } //if browser supports M$ conventions
        else {
...
function tbuttonOnClick() {
    ...
    switch (value) {
        ...
        case "table" :
            ...
            break;
        case "help" :
            window.open(cfg_filesrc + "editor_help.html", "mywindow", "resizable=1,width=300,height=500");
            break;
        case "insertimage":
