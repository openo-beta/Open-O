...
<%@ page
        import="java.util.*,oscar.oscarRx.data.*,oscar.oscarRx.pageUtil.*,java.text.DateFormatSymbols,java.io.*,org.apache.xmlrpc.*" %>
<%
...
<div id="treatment_<%=i%>"
     style="background-color:<%=sigColor(""+ht.get("significance"))%>;margin-right:10px;margin-left:20px;margin-top:10px;padding-left:10px;padding-top:10px;padding-bottom:5px;border-bottom: 2px solid gray;border-right: 2px solid #999;border-top: 1px solid #CCC;border-left: 1px solid #CCC;">
        ...
        <tr>
            ...
            <td align="right">
                <a href="javascript: function myFunction() {return false; }" onclick="$('treatmentsMyD').toggle();"
                   style="text-decoration: none;font-size:larger">X</a>
            </td>
        </tr>
        <tr>
    ...
    <%=DrugLing((String) drug.get("label"))%> <a href="javascript:void(0);"
                                                 onclick="$('searchString').value ='<%=drug.get("tc_atc")%>';$('treatmentsMyD').toggle();$('searchString').focus();"><%=drug.get("tc_atc")%>
</a> <%=drug.get("tc_atc_number")%> <br/>
    <%
            }
        }
    ...
    <%=content%>
    <%} else { %>
    <%=content.substring(0, 100)%><a id="addText_<%=i%>" style="display:none;padding:2px;"><%=content.substring(100)%>
</a>
    <div><a href="javascript:void(0);" id="addTextWord_<%=i%>" onclick="showAddText('<%=i%>');"><span>more</span></a>
    </div>
    <%}%>
</div>
...
<div style="background-color:green;margin-right:10px;margin-left:20px;margin-top:10px;padding-left:10px;padding-top:10px;padding-bottom:5px;border-bottom: 2px solid gray;border-right: 2px solid #999;border-top: 1px solid #CCC;border-left: 1px solid #CCC;">
    MyDrug to MyDrug Price Service not available -- <a href="javascript: function myFunction() {return false; }"
                                                       onclick="$('treatmentsMyD').toggle();"
                                                       style="text-decoration: none;">CLOSE</a></div>
<% } else { %>
<div style="background-color:white;margin:100px;padding:40px;border:2px solid grey">
    <a href="javascript: function myFunction() {return false; }" onclick="$('treatmentsMyD').toggle();">NOTHING
        FOUND</a>
