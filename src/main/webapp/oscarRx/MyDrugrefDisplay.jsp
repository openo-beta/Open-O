...
<span style="float: right;"><a href="javascript:void(0);"
    ...
    <%=interactStr%><br/>
    <b><%=ht.get("name")%>
    </b> From:<%=s(ht.get("author"))%> <br/>
    Last Update:<%=s(lastUpdateDate)%><br/>

    <%if (bodyStr.length() > 90) {%>
    <%=((String) ht.get("body")).substring(0, 90)%><a id="addText_<%=rand%>"
                                                      style="display:none;padding:2px;"><%=((String) ht.get("body")).substring(90)%>
</a>
    <div><a href="javascript:void(0);" id="addTextWord_<%=rand%>" onclick="showAddText('<%=rand%>');"><span>more</span></a>
    </div>
    ...
    <%}%> (<%=ht.get("evidence")%>) &nbsp;Reference: <a href="<%=ht.get("reference")%>"
                                                        target="_blank"><%= StringUtils.maxLenString(ref, 51, 50, "...") %>
</a>
    ...
    <fieldset style="border: 1px solid white; display: none; padding: 2px;"
        ...
        <div><%= getCommentDisplay(comment) %>
        </div>
        <%}%>
    </fieldset>
    <%}%>
</div>
<div id="show_<%=elementId%>"
     style="text-align: right; <%=showHidden%>font-size: 9pt; background-color:<%=sigColor(""+ht.get("significance"))%>;margin-right:3px;margin-left:3px;margin-top:2px;padding-left:3px;padding-top:3px;padding-bottom:3px;">
    <a href="javascript:void(0);"
       onclick="ShowW('<%=elementId%>','<%=ht.get("id")%>','<%=getTime(ht.get("updated_at"))%>');">Show</a>
</div>
<%
    }
    if (untrustedRes > 0) { %>
