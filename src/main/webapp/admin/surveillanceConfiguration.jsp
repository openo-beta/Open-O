    ...
    <div class="row">
            ...
            <div class="tab-content">
                <div role="tabpanel" class="tab-pane" ng-class="tabActive('local')" id="home">
                    <div class="list-group">
                        <a ng-click="openSurvey(surveyConfig)" class="list-group-item"
                           ng-class="itemActive(surveyConfig.id)"
                           data-ng-repeat="surveyConfig in loadedSurveillanceConfigs | limitTo:loadedSurveillanceConfigsQuantity">

                            <h4 class="list-group-item-heading"><span ng-if="surveyConfig.active" style="color:green"
                                                                      class="glyphicon glyphicon-ok "
                                                                      aria-hidden="true"></span> {{surveyConfig.name}}
                            </h4>
                            <p class="list-group-item-text">{{surveyConfig.date | date:'medium'}}</p>
                            <br>
                            <button ng-if="!surveyConfig.active" class="btn btn-default btn-sm"
                                    ng-click="enable(surveyConfig)">Enable
                            </button>
                            <button ng-if="surveyConfig.active" class="btn btn-default btn-sm "
                                    ng-click="disable(surveyConfig)">Disable
                            </button>
                            <button ng-if="hasUpdateAvailableFromK2A(surveyConfig.id)" class="btn btn-success btn-sm"
                                    ng-click="updateFromK2a(surveyConfig)">Update
                            </button>
        ...
        <div class="col-xs-5" ng-show="currentSurveyId != null">
            ...
            <div class="col-xs-6">
                ...
                <ol style="padding:0;">
                    <li ng-repeat="provider in survey.providerList" title="{{getFullProviderName(provider)}}">
                        {{getProviderName(provider)}} <a ng-click="removeFromSurvey(survey,provider,$index)"><i
            ...
            <div class="col-xs-6">
                ...
                <ul style="padding:0;" class="list-unstyled">
                    <li ng-repeat="provider in activeProviders" ng-if="hideIfParticipating(provider)"
                        title="{{getFullProviderName(provider)}}">
                        <a ng-click="addProviderToSurvey(survey,provider)"><i class="icon-li icon-arrow-left"></i>
                            {{getProviderName(provider.providerNo)}}</a>
                    </li>
                </ul>
            </div>
        </div>
        <div class="col-xs-4" ng-show="currentSurveyId != null">
            ...
            <form ng-if="currentSurveyActive">
                ...
                <button type="submit" class="btn btn-default"
                        ng-click="createJob(survey,jobUsername,jobPassword,prefix)">Create Job
                </button>
            ...
            <table class="table table-condensed table-striped">
                ...
                <tr ng-repeat="exportFile in exportFiles">
                    <th scope="row"><a ng-click="download(exportFile)">{{exportFile.name}}<i
                            class="icon-download-alt"></i></a></th>
                    <td>{{exportFile.createDate | date}}</td>
                    <td>
                        <button type="button" ng-if="!exportFile.sent" class="btn btn-default btn-xs"
                                ng-click="setAsSent(exportFile)">Set as Sent
                        </button>
