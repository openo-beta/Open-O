...
<div ng-controller="phrConfig" ng-cloak>
    ...
    <div class="row" data-ng-show="phrActive">
        <div class="col-xs-12">
            ...
            <div>
                <button style="margin-left:3px;" ng-repeat="recc in audit.launchItems" type="button"
                        class="btn btn-primary" ng-click="openPHRWindow(recc)">{{recc.heading}}
                </button>
                <button style="margin-left:3px;" ng-show="hasLaunchItems" type="button"
                        onclick="window.open('ApptSearchConfiguration.jsp');" class="btn btn-primary">On-line Booking
                </button>
            </div>
        </div>
    </div>
    <div class="row">
        <div class="col-xs-9">
            <div data-ng-hide="phrActive" class="jumbotron" ng-cloak>
                ...
                <div class="alert alert-warning" role="alert" data-ng-show="userpassError">Invalid Username and
                    Password.
                </div>
                <form method="POST">
                    <fieldset>
                        ...
                        <div class="form-group col-xs-10">
                            ...
                            <div class="controls">
                                <input class="form-control" name="clinicPassword" ng-model="clinicPassword"
                                       type="password" maxlength="255"/> <br/>
                            </div>
                            <button type="button" class="btn btn-primary"
                                    ng-disabled="clinicUsername==null || clinicUsername=='' || clinicPassword==null || clinicPassword=='' || working"
                                    ng-click="initPHR()"/>
            ...
            <div data-ng-hide="phrConsentConfigured || !phrActive" class="jumbotron" ng-cloak>
                ...
                <form method="POST">
                    <fieldset>
                        <h4 ng-if="!showPHRUserCreate && !showPHRUserLink">We're almost there! To complete the
                            connection, the PHR needs to register a primary OSCAR user to interact with at the system
                            level. This OSCAR user will show in OSCAR as the creator of appointments booked on-line
                            through the PHR. You have two options for registering (Option 1 strongly recommended):</h4>
                        <div class="form-group col-xs-10">
                            <h3>Set PHR User</h3>
                                ...
                                <p>Option 1 - Create a New User for PHR
                                    <small>
                                        <ul>
                                            ...
                                            <li>create a "OSCAR consent" record associated with the New User to track
                                                which patients agree to connecting with the clinic through the PHR
                                            </li>
                                            <li>automatically register this New User with the PHR to complete the
                                                connection.
                                            </li>
                                        </ul>
                                    </small>
                                    <button type="button" class="btn btn-primary btn-block" ng-click="selectPHRUser()">
                                        Create a new User
                                    </button>
                                <h3>Option 2 - Link with an Existing User</h3>
                                <small>
                                    <p>Selecting this option will create an "OSCAR consent" record associated with the
                                        selected Existing User to track which patients agree to connecting with the
                                        clinic through the PHR. To complete the connection with the PHR, you will need
                                        to manually register the Existing User in the PHR.</p>
                                </small>
                                <button type="button" class="btn btn-primary btn-block"
                                        ng-click="selectLinkExistingUser()">Link an existing user
                                </button>
                            </div>
                            <div ng-if="showPHRUserCreate">
                                <label>Provider No</label>
                                <div class="controls">
                                    <input class="form-control" name="providerNo" ng-model="newProvider.providerNo"
                                           type="text" maxlength="6"/> <br/>
                                </div>

                                <label>First Name</label>
                                <div class="controls">
                                    <input class="form-control" name="firstName" ng-model="newProvider.firstName"
                                           type="text" maxlength="25"/> <br/>
                                </div>

                                <label>Last Name</label>
                                <div class="controls">
                                    <input class="form-control" name="lastName" ng-model="newProvider.lastName"
                                           type="text" maxlength="25"/> <br/>
                                </div>
                                <label>Public OSCAR URL</label>
                                <div class="controls">
                                    <input class="form-control" name="comments" ng-model="newProvider.comments"
                                           type="text"/> <br/>
                                </div>

                                <button type="button" class="btn btn-primary btn-block" ng-click="createPHRUser()">
                                    Create a new User
                                </button>
                            </div>
                            <div ng-if="showPHRUserLink">
                                ...
                                <div class="controls">
                                    <select ng-model="newProvider.providerNo" class="form-control">
                                        <option ng-repeat="pro in activeProviders" value="{{pro.providerNo}}">
                                            {{pro.lastName}}, {{pro.firstName}} ({{pro.providerNo}})
                                        </option>
                                    </select>
                                </div>
                                ...
                                <button type="button" class="btn btn-primary btn-block" ng-click="linkExistingUser()">
                                    Link User
                                </button>
                            </div>
                        </div>
        ...
        <div class="col-xs-3">
            ...
            <div> <%-- </div>class="jumbotron" ng-if="!serverOffline && !audit.clinicInformationSetup">  --%>
                ...
                <div class="alert alert-warning" role="alert" ng-if="!phrAppointmentDataConfigured">
                    <strong>Enable Appointment Notifications For All Patients</strong> <br>If this is not enabled, the
                    EMR will only share appointment notification data for patients with a kindredPHR account
                    <button ng-click="enableAppointmentSharing()" class="btn btn-info btn-block" type="button">Enable
                    </button>
                </div>
                <div ng-repeat="recc in audit.recommendations" class="alert alert-warning" role="alert">
                    <strong>{{recc.heading}}</strong> <br>{{recc.description}}
                    <button ng-click="openPHRWindow(recc)" class="btn btn-info btn-block" type="button">Configure
                    </button>
                </div>
            </div>
        </div>
            ...
            <div class="md-dialog-content" id="dialogContentProviderCopy">
                <table class="table table-bordered table-striped table-hover">
                    ...
                    <tr ng-repeat-start="prov in vp.clinic.bookingProviders">
                        ...
                        <td>&nbsp;</td>
                        <td>
                            <button class="btn btn-primary" type="button" ng-click="vp.copyProvidersTemplate(prov)">
                                Copy
                            </button>
                        </td>
                        <td ng-repeat="appt in vp.clinic.bookingAppointmentTypes">
                            <span><small>{{vp.listAppt(prov, appt)}}</small></span>
                        </td>
                    </tr>
                    <tr ng-repeat="teamProvider in prov.teamMembers" ng-repeat-end>
                        ...
                        <td>
                            <button class="btn btn-primary" type="button"
                                    ng-click="vp.copyProvidersTemplate(teamProvider)">Copy
                            </button>
                        </td>
                        <td ng-repeat="appt in vp.clinic.bookingAppointmentTypes">
