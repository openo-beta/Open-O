...
<html>
    ...
    <%
        ...
        if (nLastPage >= 0) {
    %> <a
            href="providersearchresults.jsp?keyword=<%= Encode.forUriComponent(keyword) %>&search_mode=<%=Encode.forUriComponent(String.valueOf(searchMode))%><%=searchStatusQ%>&orderby=<%=Encode.forUriComponent(String.valueOf(orderBy))%>&limit1=<%=Encode.forUriComponent(String.valueOf(nLastPage))%>&limit2=<%=Encode.forUriComponent(String.valueOf(strLimit))%>"><fmt:message key="admin.providersearchresults.btnLastPage"/></a> | <%
        }
        if (nItems == Integer.parseInt(strLimit)) {
    %> <a
            href="providersearchresults.jsp?keyword=<%= Encode.forUriComponent(keyword) %>&search_mode=<%=Encode.forUriComponent(String.valueOf(searchMode))%><%=searchStatusQ%>&orderby=<%=Encode.forUriComponent(String.valueOf(orderBy))%>&limit1=<%=Encode.forUriComponent(String.valueOf(nNextPage))%>&limit2=<%=Encode.forUriComponent(String.valueOf(strLimit))%>"><fmt:message key="admin.providersearchresults.btnNextPage"/></a> <%
        }
    %>
