...
<a href="javascript:reportWindow('${e:forHtml(contextPath)}/lab/CA/ALL/testUploader.jsp',800,1000)" class="nav-link"><bean:message key="admin.admin.hl7LabUpload"/></a>

<c:if test="${OscarProperties.getInstance().getBooleanProperty('legacy_document_upload_enabled', 'true')}">
    <a href="javascript:reportWindow('${e:forHtml(contextPath)}/documentManager/html5AddDocuments.jsp',600,500)" class="nav-link"><bean:message key="inboxmanager.document.uploadDoc"/></a>
</c:if>
...
<a href="javascript:reportWindow('${e:forHtml(contextPath)}/documentManager/incomingDocs.jsp',800,1200)" class="nav-link"><bean:message key="inboxmanager.document.incomingDocs"/></a>

<c:if test="${!OscarProperties.getInstance().isBritishColumbiaBillingRegion()}">
    <a href="javascript:reportWindow('${e:forHtml(contextPath)}/oscarMDS/CreateLab.jsp',800,1000)" class="nav-link"><bean:message key="global.createLab" /></a>
    <a href="javascript:reportWindow('${e:forHtml(contextPath)}/olis/Search.jsp',800,1000)" class="nav-link"><bean:message key="olis.olisSearch" /></a>
