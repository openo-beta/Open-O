    ...
    <fieldset ng-repeat="mod in page.columnOne.modules">
        <legend style="margin-bottom:0px;">
            ...
            <a href="javascript:void(0)" style="font-size:12px;color:#333;padding-top:10px" class="pull-right"
               ng-click="openPreventions(demographicNo)" ng-show="mod.summaryCode=='preventions'">
                <span class="glyphicon glyphicon-plus-sign" title="{{mod.summaryCode}}"></span>
            </a>

            <a href="javascript:void(0)" style="font-size:12px;color:#333;padding-top:10px" class="pull-right"
               ng-click="gotoState('add', mod)" ng-disabled="page.cannotAdd"
            ...
            <a href="javascript:void(0)" style="font-size:12px;color:#333;padding-top:10px" class="pull-right"
               ng-click="openRx(demographicNo)" ng-show="mod.summaryCode=='meds'">
                <span class="glyphicon glyphicon-plus-sign" title="{{mod.summaryCode}}"></span>
            </a>

            <a href="javascript:void(0)" style="font-size:12px;color:#333;padding-top:10px" class="pull-right"
               ng-click="openAllergies(demographicNo)" ng-show="mod.summaryCode=='allergies'">
                <span class="glyphicon glyphicon-plus-sign" title="{{mod.summaryCode}}"></span>
            </a>

            <a href="#/record/{{demographicNo}}/forms" style="font-size:12px;color:#333;padding-top:10px"
               class="pull-right" ng-show="mod.summaryCode=='assessments'">
                <span class="glyphicon glyphicon-plus-sign" title="{{mod.summaryCode}}"></span>
            </a>
        ...
        <div ng-if="mod.status === 'ok'">
            <ul style="padding-left:12px;">
                <%-- href="{{item.action}}" --%>
                <li ng-repeat="item in mod.summaryItem" ng-show="$index < mod.displaySize"><span
                        class="pull-right">{{item.date | date: 'dd-MMM-yyyy'}}</span><a
                        ng-click="gotoState(item,mod,item.id)" href="javascript:void(0)" ng-class="item.indicatorClass"
                        uib-popover="{{item.displayName}} {{item.warning}}"
...
<div class="col-sm-6" id="middleSpace" ng-show="page.canRead" ng-click="checkAction($event)"
     ng-keypress="checkAction($event)">
    <ul class="nav nav-pills">
        <li ng-class="isCurrentStatus('none')"><a data-target="#all" ng-click="removeFilter(0)" data-toggle="tab"
                                                  class="hand-hover">All</a></li>
        <li ng-class="isCurrentStatus('Just My Notes')"><a ng-click="changeNoteFilter('Just My Notes')"
                                                           class="hand-hover">Just My Notes</a></li>
        <li ng-class="isOnlyNotesStatus()"><a ng-click="setOnlyNotes()" class="hand-hover">Just Notes</a></li>
        <li><a href="javascript:void(0)" data-target="#tracker" role="tab" data-toggle="tab"
               ng-click="getTrackerUrl(demographicNo)">Tracker</a></li>
        <li class="pull-right"><span style="margin-top:12px;" class="glyphicon glyphicon-print"
                                     ng-click="showPrintModal(page.notes.notelist)"></span></li>

    </ul>
    <div class="tab-content">
        <div class="tab-pane active" id="all">
            <dl infinite-scroll="addMoreItems()">
                <dt ng-style="setColor(note)" ng-repeat-start="note in page.notes.notelist"
                    ng-show="showNoteHeader(note)">
                    ...
                    </div --%>{{note.observationDate | date: 'dd-MMM-yyyy'}} {{firstLine(note)}} <input
                        type="checkbox" ng-model="note.isSelected" class="pull-right" style="margin-right:3px;"/></dt>
