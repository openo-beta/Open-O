...
<fieldset>
    <legend style="margin-bottom:0px;">Current and Long Term
        <button type="button" class="btn btn-default btn-xs" ng-class="$ctrl.buttonStyle(1)"
                ng-click="$ctrl.setMode(1)">All
        </button>
        <button type="button" class="btn btn-primary btn-xs" ng-class="$ctrl.buttonStyle(0)"
                ng-click="$ctrl.setMode(0)">Current
        </button>
        <div class="row">
            <div class="col-sm-12">
                <div class="input-group">
                    <input type="text" class="form-control" placeholder="Search Drug Profile"
                           ng-model="$ctrl.drugProfileFilter">
        ...
        <tr ng-repeat="drug in $ctrl.rxComp.drugs | filter:$ctrl.drugProfileFilter | orderBy: '-rxDate'"
            ...
            <td ng-class="{ 'deletedItem': drug.archived }">
                ...
                <div ng-repeat="warning in $ctrl.dsMessagesHash[drug.atc]">
                    <a ng-hide="$ctrl.checkIfHidden(warning)"
                       ng-class="'alert-' + ($ctrl.getAlertStyl(warning) || 'warning')"
                       ng-click="$ctrl.showAlert({'alert':warning})"><span <%-- style="color:red" --%>
                            class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span>
                        {{$ctrl.getHeading(warning)}}</a>
                </div>
            </td>
            <td>{{drug.rxDate}}</td>
            ...
            <td>
                <div class="btn-group">
                    ...
                    <ul class="dropdown-menu">
                        ...
                        <li ng-if="!drug.longTerm"><a ng-click="$ctrl.setAsLongTermMed(drug)">Set as Long Term Med</a>
                        </li>
                        <li ng-if="drug.longTerm"><a ng-click="$ctrl.unsetAsLongTermMed(drug)">Unset as Long Term
                            Med</a></li>
    ...
    <ul class="nav nav-tabs nav-justified" role="tablist">
        <li role="presentation" ng-class="{'active' : $ctrl.booleanTab(0)}" <%-- class="{{$ctrl.activeTab(0)}}" --%>><a
                ng-click="$ctrl.setTab(0)" aria-controls="expired" role="tab" data-toggle="tab">Expired Medications</a>
        </li>
        <li role="presentation" ng-class="{'active' : $ctrl.booleanTab(1)}"><a ng-click="$ctrl.setTab(1)"
                                                                               aria-controls="discontinued" role="tab"
                                                                               data-toggle="tab">Discontinued
            Medications</a></li>
    </ul>
        ...
        <div role="tabpanel" class="tab-pane" ng-class="{'active' : $ctrl.booleanTab(0)}">
            ...
            <table class="table table-condensed table-striped">
                ...
                <tr ng-repeat="drug in $ctrl.rxComp.drugs | filter:$ctrl.drugProfileFilter | orderBy: '-rxDate'"
                    ...
                    <td ng-class="{ 'deletedItem': drug.archived }">
                        <a ng-click="$ctrl.medhistory(drug)">{{drug.instructions}}</a>
                        <div ng-if="drug.externalProvider"><em>External Prescribed by {{drug.externalProvider}}</em>
                        </div>
                        <div ng-repeat="warning in $ctrl.dsMessagesHash[drug.atc]">
                            <a ng-hide="$ctrl.checkIfHidden(warning)"
                               ng-class="'alert-' + ($ctrl.getAlertStyl(warning) || 'warning')"
