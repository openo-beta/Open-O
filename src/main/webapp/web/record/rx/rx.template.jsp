...
<div class="col-sm-3">
    <fieldset ng-repeat="mod in $ctrl.page.columnOne.modules">
        <legend style="margin-bottom:0px;">
            ...
            <a href="javascript:void(0)" style="font-size:12px;color:#333;padding-top:10px" class="pull-right"
               ng-click="$ctrl.openAllergies()" ng-show="mod.summaryCode=='allergies'">
        ...
        <ul style="padding-left:12px;">
            ...
            <li ng-repeat="item in mod.summaryItem | orderBy: displayName" ng-show="$index < mod.displaySize">
                ...
                <a ng-click="$ctrl.gotoState(item,mod,item.id)" href="javascript:void(0)" ng-class="item.indicatorClass"
...
<div class="col-sm-6" id="middleSpace" ng-click="checkAction($event)" ng-keypress="checkAction($event)">
    ...
    <div class="row">
        <ul class="nav nav-pills nav-justified">
            <li class="active" <%-- %>ng-class="isCurrentStatus('none')" --%> ><a data-target="#all"
                                                                                  ng-click="removeFilter(0)"
                                                                                  data-toggle="tab" class="hand-hover">Prescribe</a>
            </li>
            <li ng-class="isCurrentStatus('Just My Notes')"><a ng-click="changeNoteFilter('Just My Notes')"
                ...
                <div class="row">
                    <div class="col-sm-8">
			  		<span class="pull-right">
			  			...
			  			<a ng-click="$ctrl.cancelMed(med,$index)"><span
                                class="glyphicon glyphicon-remove-circle"></span></a>
			  		</span>
                        <h4 class="list-group-item-heading">{{med.getName()}}</h4>
                        <form>
                            <div class="form-group">
                                <input type="text" class="form-control" ng-if="med.custom" id="customInput"
                                       placeholder="Custom Drug Name" ng-model="med.customName" auto-focus>
                            </div>
                            <div class="form-group">
                                ...
                                <div class="row">
                                    <div class="col-xs-6" ng-class="$ctrl.checkForQuantityError(med)">
                                        ...
                                        <input type="text" class="form-control" id="quantityInput"
                                               ...
                                               ng-change="$ctrl.manualQuantityEntry(med)">
                                        <span ng-if="$ctrl.checkForQuantityError(med)" id="helpBlock2"
                                              class="help-block">Quantity was not calculated. Manual Calculation required</span>
                                    </div>
                                    <div class="col-xs-6">
                                        <label for="repeatsInput">Repeat</label>
                                        <input type="text" class="form-control" id="repeatsInput" placeholder="Repeats"
                    ...
                    <div class="col-sm-4">
                        <div class="alert alert-danger" ng-if="med.custom"><b>Warning</b> you will lose the following
                            functionality:
                        ...
                        <div uib-alert ng-repeat="alert in $ctrl.page.dsMessageHash[med.atc]" class="alert"
                             ng-class="'alert-' + ($ctrl.getAlertStyl(alert) || 'warning')"
                             style="padding: 9px;margin-bottom: 3px;"
                             ng-hide="$ctrl.checkIfHidden(alert)">  <%-- uib-popover-html="{{alert.body}}" popover-trigger="'mouseenter'"  --%>
                            {{alert.heading}}<br>
        ...
        <medsearch med-selected="$ctrl.medSelected(med)" fav-selected="$ctrl.favSelected(fav)"
                   favourite-meds="$ctrl.page.favouriteDrugs" custom-rx="$ctrl.customRx(nam)"></medsearch>
        <button type="button" class="btn btn-primary btn-block" style="margin-top:3px;" ng-click="$ctrl.saveAndPrint()">
            Save And Print
        </button>
    </div>
    <hr>
    <div class="row">
        <rx-profile fulldrugs="$ctrl.page.fulldrugs" re-rx="$ctrl.reRx(drug)" ds-messages="$ctrl.page.dsMessageHash"
                    show-alert="$ctrl.showAlert(alert)" add-favorite="$ctrl.addToFavourite(drug)"></rx-profile>
    </div>
...
<div class="col-sm-3">
    ...
    <fieldset ng-repeat="mod in $ctrl.page.columnThree.modules">
        ...
        <ul style="padding-left:12px;">
            <%-- href="{{item.action}}" --%>
            <li ng-repeat="item in mod.summaryItem | filter:incomingQ" ng-show="$index < mod.displaySize"><span
                    class="pull-right">{{item.date | date: 'dd-MMM-yyyy'}}</span><a ng-click="gotoState(item)"
                                                                                    class="hand-hover"
