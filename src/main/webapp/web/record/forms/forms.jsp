...
<div class="col-sm-3">
    <ul class="nav nav-tabs nav-justified">
        <li ng-repeat="list in page.formlists" ng-class="getListClass(list.id)" class="hand-hover"><a
                ng-click="changeTo(list.id)">{{list.label}}</a></li>
        ...
        <li class="dropdown">
            ...
            <ul class="dropdown-menu">
                <li ng-show="hasAdminAccess"><a class="hand-hover"
                                                onclick="popup(600, 1200, '../administration/?show=Forms', 'manageeforms')">Manage
                    eForms</a></li>
                <li ng-show="hasAdminAccess"><a class="hand-hover"
                                                onclick="popup(600, 1200, '../administration/?show=Forms&load=Groups', 'editGroups')">Edit
                    Groups</a></li>
    ...
    <div class="panel panel-success">
        ...
        <ul class="list-group" tabindex="0" ng-keypress="keypress($event)">
            <li class="list-group-item"
                ng-repeat="item in page.currentFormList[page.currentlistId] | filter:filterFormsQ"
                ng-class="getActiveFormClass(item)">
                <input type="checkbox" ng-model="item.isChecked"/>
                <a class="list-group-item-text hand-hover" title="{{item.subject}}"
                   ng-click="viewFormState(item,1)"><span ng-show="item.date"
                                                          class="pull-right">{{item.date | date: 'd-MMM-y'}}</span>{{item.name}}</a>
            </li>

            <li class="list-group-item"
                ng-repeat="formItem2 in page.encounterFormlist[page.currentlistId] | filter:filterFormsQ"
                ng-hide="page.currentlistId==1">
                <a class="list-group-item-text hand-hover" ng-click="viewFormState(formItem2,1)">{{formItem2.name}}</a>
            </li>

            <li class="list-group-item"
                ng-repeat="formItem in page.encounterFormlist[page.currentlistId] | filter:filterFormsQ"
                ng-hide="page.currentlistId==0">
                <a class="list-group-item-text hand-hover" ng-click="viewFormState(formItem,1)">{{formItem.formName}}
                    <span ng-show="formItem.date" class="pull-right">{{formItem.date | date: 'd-MMM-y'}}</span></a>
            </li>
            ...
            <div class="col-sm-4">
                ...
                <ul class="list-group">
                    <li ng-repeat="mod in page.formGroups" class="list-group-item" ng-class="getGroupListClass(mod)">
                        ...
                        <a class="list-group-item-text" ng-click="setCurrentEFormGroup(mod)" href="javascript:void(0)">
                            {{mod.displayName}} </a>
                    </li>
                </ul>
            </div>
