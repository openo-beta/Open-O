...
<div class="add-document-wrapper">
	...
	<div class="form-group">
		...
		<input type="text" class="form-control <c:if test='${ docerrors["descmissing"] != null}' >alert-danger</c:if>" id="docDesc" name="docDesc" value="<%=formdata.getDocDesc()%>" onfocus="checkDefaultValue(this)" />
	...
	<% if (module.equals("provider")) {%>
		<div class="checkbox">
			<label>
			<input type="checkbox" id="docPublic" name="docPublic" <%=formdata.getDocPublic() + " "%> value="checked">
			Public</label>
		</div>
	<% } %>
	<div class="form-group">
	...
	<div id="addLinkDiv" class="addDocDiv form-inline well" style="display: none;">
		...
		<input type="text" name="docDesc" id="docDesc2" class="form-control <c:if test="${ linkhtmlerrors['descmissing'] != null }">alert-danger</c:if>"
