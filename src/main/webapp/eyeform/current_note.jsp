...
function saveEyeformNote() {
    ...
    jQuery.ajax({
        url: ctx + "/eyeform/FollowUp.do?method=getNoteText&appointmentNo=" +<%=aptNo%>,
        async: false,
    ...
    jQuery.ajax({
        url: ctx + "/eyeform/ProcedureBook.do?method=getNoteText&appointmentNo=" +<%=aptNo%>,
        async: false,
        success: function (data) {
            notetext += data;
            //if(data.length>0) {notetext+='\n';}
    ...
    jQuery.ajax({
        url: ctx + "/eyeform/TestBook.do?method=getNoteText&appointmentNo=" +<%=aptNo%>,
        async: false,
        success: function (data) {
            notetext += data;
            //if(data.length>0) {notetext+='\n';}
    ...
    jQuery.ajax({
        url: ctx + "/eyeform/NoteData.do?method=getNoteText&appointmentNo=" +<%=aptNo%>,
        async: false,
        success: function (data) {
            notetext += data;
            // if(data.length>0) {notetext+='\n';}
...
function saveNoteAndSendTickler() {
    ...
    //get consults/procedures/tests/checkboxes to generate text
    jQuery.ajax({
        url: ctx + "/eyeform/FollowUp.do?method=getTicklerText&appointmentNo=" +<%=aptNo%>,
        async: false,
        success: function (data) {
            notetext += data;
            if (data.length > 0) {
    ...
    jQuery.ajax({
        url: ctx + "/eyeform/ProcedureBook.do?method=getTicklerText&appointmentNo=" +<%=aptNo%>,
        async: false,
        success: function (data) {
            notetext += data;
            if (data.length > 0) {
    ...
    jQuery.ajax({
        url: ctx + "/eyeform/TestBook.do?method=getTicklerText&appointmentNo=" +<%=aptNo%>,
        async: false,
        success: function (data) {
            notetext += data;
            if (data.length > 0) {
    ...
    jQuery.ajax({
        url: ctx + "/eyeform/NoteData.do?method=sendTickler&appointmentNo="+<%=aptNo%>+
        "&text="+notetext + "&recip=" + ticklerRecip.value + "&demographicNo=<%=demographicNo %>",
        async: false,
        success: function (data) {
            alert('tickler sent');
        }
...
<span><input type="button" onclick="popupPageOne('<c:out
        value="${ctx}"/>/eyeform/EyeformPlan.do?method=form&amp;followup.demographicNo=<%=demographicNo %>&amp;noteId=<%=noteId%>&amp;followup.appointmentNo=<%=aptNo%>','eyeFormPlan',600,1200);"
             value="Arrange Plan"/></span>
	...
	<c:forEach items="${followUps}" var="item">
        <c:choose>
            <c:when test="${item.timespan == 0}">
                <span style="font-size:10pt"><c:out value="${item.typeStr}"/>&nbsp;Dr.&nbsp;<c:out
                        value="${item.provider.firstName}"/>&nbsp;<c:out value="${item.provider.lastName}"/> | <c:out
                        value="${item.urgency}"/> | <span title="<c:out value="${item.comment}"/>"><c:out
                        value="${item.commentStr}"/></span></span>
            </c:when>
            <c:otherwise>
                <span style="font-size:10pt"><c:out value="${item.typeStr}"/>&nbsp;<c:out value="${item.timespan}"/>&nbsp;<c:out
                        value="${item.timeframe}"/>&nbsp;Dr.&nbsp;<c:out
                        value="${item.provider.firstName}"/>&nbsp;<c:out value="${item.provider.lastName}"/> | <c:out
                        value="${item.urgency}"/> | <span title="<c:out value="${item.comment}"/>"><c:out
                        value="${item.commentStr}"/></span></span>
            </c:otherwise>
...
<div>
	...
	<c:forEach items="${procedures}" var="item">
        <span style="font-size:10pt"><c:out value="${item.eye}"/>&nbsp;<c:out value="${item.procedureName}"/> at <c:out
                value="${item.location}"/> | <c:out value="${item.urgency}"/> | <span
                title="<c:out value="${item.comment}"/>"><c:out value="${item.commentStr}"/></span></span><br/>
    </c:forEach>
...
<div>
	...
	<c:forEach items="${testBookRecords}" var="item">
        <span style="font-size:10pt"><c:out value="${item.eye}"/>&nbsp;<c:out value="${item.testname}"/> | <c:out
                value="${item.urgency}"/> | <span title="<c:out value="${item.comment}"/>"><c:out
                value="${item.commentStr}"/></span></span><br/>
    </c:forEach>
...
<div>
        <div>
           ...
           <input tabindex="251" value="Generate Note" onclick="saveEyeformNote();return false;" id="stickler0"
           ...
           <td width="15%" nowrap="nowrap">            
			<input tabindex="251" value="Send Tickler" onclick="saveNoteAndSendTickler();return false;" id="stickler"
                   style="color: black;" type="button">
