...
// get total patientNum/rosterNR/rosterRO/sexF/sexM/
    ...
    String[] dateParams = new String[]{sdate, edate};
// get nurse name list
...
// get service code list
    ...
    rs = db.queryResults(sql, dateParams);
    while (rs.next()) {
        vServiceCode.add(ca.openosp.Misc.getString(rs, "service_code"));
        vServiceDesc.add(ca.openosp.Misc.getString(rs, "service_desc"));
    }

    for (int i = 0; i < vServiceCode.size(); i++) {
        // get total pat
        sql = "select count(distinct(b.demographic_no)) from billing b, billingdetail bd where b.billing_no=bd.billing_no  and b.billing_date>=? and b.billing_date<=? and b.status!='D' and bd.status!='D' and bd.service_code=? and bd.service_desc=?";
        rs = db.queryResults(sql, new String[]{sdate, edate, (String) vServiceCode.get(i), (String) vServiceDesc.get(i)});
        while (rs.next()) {
            props.setProperty(vServiceCode.get(i) + "pat" + vServiceDesc.get(i), ca.openosp.Misc.getString(rs, "count(distinct(b.demographic_no))"));
        }
...
*/
        ...
        // get total vis
        sql = "select count(distinct(b.billing_no)) from billing b, billingdetail bd where b.billing_no=bd.billing_no  and b.billing_date>=? and b.billing_date<=? and b.status!='D' and bd.status!='D' and bd.service_code=? and bd.service_desc=?";
        rs = db.queryResults(sql, new String[]{sdate, edate, (String) vServiceCode.get(i), (String) vServiceDesc.get(i)});
        while (rs.next()) {
            props.setProperty(vServiceCode.get(i) + "vis" + vServiceDesc.get(i), ca.openosp.Misc.getString(rs, "count(distinct(b.billing_no))"));
        }
...
*/
        ...
        // get pat Physician
        sql = "select count(distinct(b.demographic_no)) from billing b, billingdetail bd, provider p where b.billing_no=bd.billing_no and b.creator=p.provider_no and b.billing_date>=? and b.billing_date<=? and b.status!='D' and bd.status!='D' and bd.service_code=? and  p.provider_type='doctor' and bd.service_desc=?";
        rs = db.queryResults(sql, new String[]{sdate, edate, (String) vServiceCode.get(i), (String) vServiceDesc.get(i)});
        while (rs.next()) {
...
*/
        ...
        // get vis Physician
        sql = "select count(distinct(b.billing_no)) from billing b, billingdetail bd, provider p where b.billing_no=bd.billing_no and b.creator=p.provider_no and b.billing_date>=? and b.billing_date<=? and b.status!='D' and bd.status!='D' and bd.service_code=? and  p.provider_type='doctor' and bd.service_desc=?";
        rs = db.queryResults(sql, new String[]{sdate, edate, (String) vServiceCode.get(i), (String) vServiceDesc.get(i)});
        while (rs.next()) {
...
*/
        ...
        // get pat Resident
        sql = "select count(distinct(b.demographic_no)) from billing b, billingdetail bd, provider p where b.billing_no=bd.billing_no and b.creator=p.provider_no and b.billing_date>=? and b.billing_date<=? and b.status!='D' and bd.status!='D' and bd.service_code=? and  p.provider_type='resident' and bd.service_desc=?";
        rs = db.queryResults(sql, new String[]{sdate, edate, (String) vServiceCode.get(i), (String) vServiceDesc.get(i)});
        while (rs.next()) {
...
*/
        ...
        // get vis Resident
        sql = "select count(distinct(b.billing_no)) from billing b, billingdetail bd, provider p where b.billing_no=bd.billing_no and b.creator=p.provider_no and b.billing_date>=? and b.billing_date<=? and b.status!='D' and bd.status!='D' and bd.service_code=? and  p.provider_type='resident' and bd.service_desc=?";
        rs = db.queryResults(sql, new String[]{sdate, edate, (String) vServiceCode.get(i), (String) vServiceDesc.get(i)});
        while (rs.next()) {
...
*/
        ...
        for (int j = 0; j < vNurseNo.size(); j++) {
            sql = "select count(distinct(b.demographic_no)) from billing b, billingdetail bd  where b.billing_no=bd.billing_no and bd.service_code=? and b.creator=? and b.billing_date>=? and b.billing_date<=? and b.status!='D' and bd.status!='D' and bd.service_desc=?";
            rs = db.queryResults(sql, new String[]{(String) vServiceCode.get(i), (String) vNurseNo.get(j), sdate, edate, (String) vServiceDesc.get(i)});
            while (rs.next()) {
                props.setProperty(vServiceCode.get(i) + "patNurse" + j + vServiceDesc.get(i), ca.openosp.Misc.getString(rs, "count(distinct(b.demographic_no))"));
            }
/*
		out.println("<hr>");
...
*/
            ...
            sql = "select count(distinct(b.billing_no)) from billing b, billingdetail bd  where b.billing_no=bd.billing_no and bd.service_code=? and b.creator=? and b.billing_date>=? and b.billing_date<=? and b.status!='D' and bd.status!='D' and bd.service_desc=?";
            rs = db.queryResults(sql, new String[]{(String) vServiceCode.get(i), (String) vNurseNo.get(j), sdate, edate, (String) vServiceDesc.get(i)});
            while (rs.next()) {
                props.setProperty(vServiceCode.get(i) + "visNurse" + j + vServiceDesc.get(i), ca.openosp.Misc.getString(rs, 1));
            }
/*
		out.println("<hr>");
