...
<display:table class="simple" cellspacing="2" cellpadding="3"
    ...
    <display:column sortable="false">
        <caisi:isModuleLoad moduleName="TORONTO_RFQ" reverse="true">
            <input type="button" value="Reject"
                   <c:if test="${queue_entry.headClientId != null || requestScope.userIsProgramProvider != 'true'}">disabled</c:if>
                   onclick="select_client('<c:out value="${queue_entry.clientId}"/>','reject','<c:out
                           value="${queue_entry.id}"/>')"/>
        </caisi:isModuleLoad>
    ...
    <%--<display:column sortable="false">--%>
    <!--<a href="javascript:void(0)" title="Case management" onclick="cme_client('<c:out
        value="${queue_entry.programId}"/>', '<c:out value="${queue_entry.clientId}"/>')">-->
...
<c:if test="${requestScope.do_admit != null}">
    <table width="100%" border="1" cellspacing="2" cellpadding="3">
        ...
        <tr>
            ...
            <td><input type="text" id="admissionDate" name="admissionDate" value="" readonly><img titltype="text"
                                                                                                  e="Calendar"
                                                                                                  id="cal_admissionDate"
