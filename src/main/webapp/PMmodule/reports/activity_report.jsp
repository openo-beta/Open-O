...
<h1>Activity Report
    : <%=functionalCentre == null ? "" : functionalCentre.getDescription()%> <%=program == null ? "" : program.getName()%>
    from <%=startDateString%> to <%=endDateString%>
</h1>
<div>
    The numbers here represent the number of encounters/notes except in the total unique clients column where it
    represents clients.
    The total and subtotal rows don't always add up to all the encounter types as some encounters are left blank for the
    encounter type.
    The total encounters column is a unique encounter count, so if an encounter involved 3 issues, it will only be
    counted once in the total unique encounters column.
    The total unique clients column is a unique count so if a client had 10 encounters for 15 issues, they will only be
    counted once in the total unique client column
</div>
...
<table class="genericTable">
    <tr class="genericTableRow">
        ...
        %>
        <td class="genericTableHeader"><%=issueGroup.getName()%>
        </td>
        <%
            }
        %>
    ...
    %>
    <tr class="genericTableRow">
        <td class="genericTableHeader"><%=tempRoleName%>
        </td>
        <td class="genericTableData"><%=encounterEntry.getKey().name()%>
        </td>
        <%
            for (Integer issueGroupEntry : encounterEntry.getValue().values()) {
        %>
        <td class="genericTableData"><%=issueGroupEntry%>
        </td>
        <%
            }
        %>

        <td class="genericTableData"><%=encounterEntry.getValue().rowTotalUniqueEncounters%>
        </td>
        <td class="genericTableData"><%=encounterEntry.getValue().rowTotalUniqueClients%>
        </td>
    </tr>
    <%
        }
    ...
    <tr class="genericTableRow">
        ...
        %>
        <td class="genericTableData"><%=encounterTypeDataRow.rowTotalUniqueEncounters%>
        </td>
        <td class="genericTableData"><%=encounterTypeDataRow.rowTotalUniqueClients%>
        </td>
    </tr>
    <%
        }
