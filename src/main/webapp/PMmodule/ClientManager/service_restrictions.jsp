...
<display:table class="simple" cellspacing="2" cellpadding="3" id="service_restriction" name="serviceRestrictions"
               export="false" pagesize="0" requestURI="/PMmodule/ClientManager.do">
    <%
    ...
    <display:column sortable="true" title="">
        <input type="button" <%=allowTerminateEarly ? "" : "disabled=\"disabled\""%> value="Terminate Early"
               onclick="terminateEarly(<%=temp.getId()%>)"/>
    </display:column>
</display:table>
<br/>
<br/>
...
<table>
    <tr>
        ...
        <td align="center"><input type="button" name="reset" value="reset" onclick="javascript:resetClientFields();"/>
        </td>
    </tr>
</table>
<br/>
<c:if test="${requestScope.do_restrict != null}">
    <table class="b" border="0" width="100%">
        ...
        <tr>
            ...
            <td><c:out value="${program.numOfMembers}"/>/<c:out value="${program.maxAllowed}"/>&nbsp;(<c:out
            ...
            <table width="100%" border="1" cellspacing="2" cellpadding="3">
                ...
                <tr class="b">
                    <td colspan="2"><input type="button" value="Add Service Restriction"
                                           onclick="do_service_restriction()"/> <input type="button" value="Cancel"
                                                                                       onclick="document.clientManagerForm.submit()"/>
                    </td>
                </tr>
            </table>
        </c:when>
        <c:otherwise>
            <b style="color: red">You do not have permission to apply service restrictions to "<c:out
                    value="${program.name}"/>."</b>
        </c:otherwise>
    </c:choose>
</c:if>
