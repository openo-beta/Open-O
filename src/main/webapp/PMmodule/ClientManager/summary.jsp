...
<script>
    ...
    function openSurvey() {
        ...
        location.href = "<%=request.getContextPath() %>/PMmodule/Forms/SurveyExecute.do?method=survey&formId=" + encodeURIComponent(formId) + "&formInstanceId=" + encodeURIComponent(id) + "&clientId=" + '<c:out value="${client.demographicNo}"/>';
    }
...
<table class="simple" cellspacing="2" cellpadding="3">
    ...
    <caisi:isModuleLoad moduleName="TORONTO_RFQ" reverse="true">
        <tr>
            ...
            <td colspan="2">
                <c:out value="${client.hin}"/>&nbsp;<c:out value="${client.ver}"/>&nbsp;(<c:out
                    value="${client.hcType}"/>)
                <%
                ...
                %>
                <input type="button" value="Manage Health Number Registry"
                       onclick="document.location='ClientManager/manage_hnr_client.jsp?demographicId=<%=currentDemographic.getDemographicNo()%>'"/>
                <%
                    }
                %>
            </td>
        ...
        <tr>
            ...
            <td colspan="2">
                ...
                %>
                <a href="javascript:void(0);" onclick="window.open('<c:out
                        value="${ctx}"/>/demographic/demographiccontrol.jsp?displaymode=edit&dboperation=search_detail&demographic_no=
                    <c:out value="${demographicNo}"/>','master_file');return false;">OSCAR Master File</a>
                <%
                    }
                %>
            </td>
                ...
                <c:choose>
                    ...
                    <c:otherwise>
                        ...
                        <tr>
                            ...
                            <td width="30%">Date: <fmt:formatDate value="${healthsafety.updateDate}"
    ...
    <tr>
        ...
        <td colspan="2">
            ...
            %>
            <input type="button" <%=isIntegratorContactable ? "" : "disabled=\"disabled\""%> value="Change Consent"
                   onclick="document.location='ClientManager/manage_consent.jsp?demographicId=<%=currentDemographic.getDemographicNo()%>'"/> <%=consentString%>
        </td>
    </tr>
    <tr>
        <th width="20%">Linked clients</th>
        <td colspan="2"><input type="button" <%=isIntegratorContactable ? "" : "disabled=\"disabled\""%>
                               value="Manage Linked Clients"
                               onclick="document.location='ClientManager/manage_linked_clients.jsp?demographicId=<%=currentDemographic.getDemographicNo()%>'"/>
        </td>
    </tr>
    <%
        }
...
<table class="simple" cellspacing="2" cellpadding="3">
    <c:choose>
        <c:when test="${relations == null}">
            <tr>
                <td><span style="color: red">No Family Members Registered</span> <input type="button" value="Update"
                    ...
                    <td><c:choose>
                        <c:when test="${rHash['dependent'] == null}">
                            <c:if test="${rHash['dependentable'] != null}">
                                Add as
                                <input type="button"
                                       onclick="saveJointAdmission('<c:out value="${rHash['demographicNo']}"/>','<c:out
                                               value="${client.demographicNo}"/>','2')" value="dependent"/>
                                <input type="button"
                                       onclick="saveJointAdmission('<c:out value="${rHash['demographicNo']}"/>','<c:out
                                               value="${client.demographicNo}"/>','1')" value="spouse"/>
                            </c:if>
                        </c:when>
                        <c:when test="${rHash['dependent'] == 2}">
                            Dependent <input type="button"
                                             onclick="removeJointAdmission('<c:out
                                                     value="${rHash['demographicNo']}"/>')"
                                             value="remove"/>
                        </c:when>
        ...
        <c:when test="${bedDemographic != null}">
            <tr>
                ...
                <td><c:out value="${bedDemographic.roomName}"/> (<c:out
                        value="${bedDemographic.programName}"/>)
                </td>
            </tr>
            <tr>
                <th width="20%">Assigned Bed:</th>
...
<table class="simple" cellspacing="2" cellpadding="3">
    ...
    <tr>
        ...
        <td><%=cdsClientForm.isSigned() ? "signed" : "unsigned"%>
        </td>
        <td>
            <input type="button" value="Update"
                   onclick="document.location='ClientManager/cds_form_4.jsp?cdsFormId=<%=cdsClientForm.getId()%>'"/>
            <input type="button" value="Print Preview"
                   onclick="document.location='ClientManager/cds_form_4.jsp?cdsFormId=<%=cdsClientForm.getId()%>&print=true'"/>
        </td>
    </tr>
    <%
            }
    ...
    <tr>
        <td colspan="5">
            <input type="button" value="New CDS Form"
                   onclick="document.location='ClientManager/cds_form_4.jsp?demographicId=<%=currentDemographic.getDemographicNo()%>'"/>
        </td>
    </tr>
    <%
    ...
    <tr>
        ...
        <c:if test="${ocanStaffForm != null}">
            ...
            <td>
                <input type="button" value="Update"
                       onclick="document.location='ClientManager/ocan_form.jsp?ocanType=FULL&demographicId=<%=currentDemographic.getDemographicNo()%>'"/>
                <input type="button" value="Print Preview"
        ...
        <c:if test="${ocanStaffForm == null}">
            ...
            <td></td>
            <td>
                <input type="button" value="New FULL OCAN Form"
                       onclick="document.location='ClientManager/ocan_form.jsp?prepopulate=0&ocanType=FULL&demographicId=<%=currentDemographic.getDemographicNo()%>'"/>
                <input type="button" value="New FULL OCAN Form - Prepopulated"
    ...
    <tr>
        ...
        <c:if test="${ocanStaffForm != null}">
            ...
            <td>
                <input type="button" value="Update"
                       onclick="document.location='ClientManager/ocan_client_form.jsp?ocanType=FULL&demographicId=<%=currentDemographic.getDemographicNo()%>'"/>
                <input type="button" value="Print Preview"
                       onclick="document.location='ClientManager/ocan_client_form.jsp?ocanType=FULL&demographicId=<%=currentDemographic.getDemographicNo()%>&print=true'"/>
            </td>
        </c:if>
        <c:if test="${ocanStaffForm == null}">
            <td><span style="color: red">None found</span></td>
            ...
            <td>
                <input type="button" value="New FULL OCAN Form"
                       onclick="document.location='ClientManager/ocan_client_form.jsp?prepopulate=0&ocanType=FULL&demographicId=<%=currentDemographic.getDemographicNo()%>'"/>
            </td>
            <td>
            </td>
        </c:if>
    ...
    <tr>
        ...
        <c:if test="${selfOcanStaffForm != null}">
            ...
            <td>
                <input type="button" value="Update"
                       onclick="document.location='ClientManager/ocan_form.jsp?ocanType=SELF&demographicId=<%=currentDemographic.getDemographicNo()%>'"/>
                <input type="button" value="Print Preview"
        ...
        <c:if test="${selfOcanStaffForm == null}">
            ...
            <td></td>
            <td>
                <input type="button" value="New SELF+CORE OCAN Form"
                       onclick="document.location='ClientManager/ocan_form.jsp?prepopulate=0&ocanType=SELF&demographicId=<%=currentDemographic.getDemographicNo()%>'"/>
                <input type="button" value="New SELF+CORE OCAN Form - Prepopulated"
    ...
    <tr>
        ...
        <c:if test="${selfOcanStaffForm != null}">
            ...
            <td>
                <input type="button" value="Update"
                       onclick="document.location='ClientManager/ocan_client_form.jsp?ocanType=SELF&demographicId=<%=currentDemographic.getDemographicNo()%>'"/>
                <input type="button" value="Print Preview"
        ...
        <c:if test="${selfOcanStaffForm == null}">
            ...
            <td>
                <input type="button" value="New SELF+CORE OCAN Form"
                       onclick="document.location='ClientManager/ocan_client_form.jsp?prepopulate=0&ocanType=SELF&demographicId=<%=currentDemographic.getDemographicNo()%>'"/>
            </td>
            <td></td>
        </c:if>
    </tr>
    ...
    <tr>
        ...
        <c:if test="${coreOcanStaffForm != null}">
            ...
            <td>
                <input type="button" value="Update"
                       onclick="document.location='ClientManager/ocan_form.jsp?ocanType=CORE&demographicId=<%=currentDemographic.getDemographicNo()%>'"/>
                <input type="button" value="Print Preview"
        ...
        <c:if test="${coreOcanStaffForm == null}">
            ...
            <td></td>
            <td>
                <input type="button" value="New CORE OCAN Form"
                       onclick="document.location='ClientManager/ocan_form.jsp?prepopulate=0&ocanType=CORE&demographicId=<%=currentDemographic.getDemographicNo()%>'"/>

                <input type="button" value="New CORE OCAN Form - Prepopulated"
    ...
    <tr>
        ...
        <td>
            <input type="button" value="Update"
                   onclick="document.location='ClientManager/cbi_form.jsp?ocanStaffFormId=<%=cbiForm.getId()%>&ocanType=CBI&demographicId=<%=currentDemographic.getDemographicNo()%>'"/>
    ...
    <tr>
        <td colspan="5">
            <input type="button" value="New CBI Form"
                   onclick="document.location='ClientManager/cbi_form.jsp?ocanStaffFormId=0&prepopulate=0&ocanType=CBI&demographicId=<%=currentDemographic.getDemographicNo()%>'"/>
        </td>
    </tr>
    <%} %>
</table>
...
<display:table class="simple" cellspacing="2" cellpadding="3" id="admission" name="admissions" export="false"
               pagesize="10" requestURI="/PMmodule/ClientManager.do">
...
<display:table class="simple" cellspacing="2" cellpadding="3" id="referral" name="referrals" export="false"
               pagesize="10" requestURI="/PMmodule/ClientManager.do">
    <display:setProperty name="paging.banner.placement" value="bottom"/>
