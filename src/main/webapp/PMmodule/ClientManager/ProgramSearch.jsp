...
<html>
    ...
    <script type="text/javascript">
        ...
        .<%=request.getParameter("formName")%>.
            elements['<%=request.getParameter("formElementId")%>'].value = id;
            var facilityId = opener.document
        .<%=request.getParameter("formName")%>.
            elements['referral.remoteFacilityId'];
            if (facilityId) {
                facilityId.value = null;
            }
            var programId = opener.document
        .<%=request.getParameter("formName")%>.
            elements['referral.remoteProgramId'];
            if (programId) {
        ...
        function selectRemoteProgram(facilityId, facilityProgramId) {
            ...
            opener.document
        .<%=request.getParameter("formName")%>.
            elements['<%=request.getParameter("formElementId")%>'].value = 0;
            opener.document
        .<%=request.getParameter("formName")%>.
            elements['referral.remoteFacilityId'].value = facilityId;
            opener.document
        .<%=request.getParameter("formName")%>.
            elements['referral.remoteProgramId'].value = facilityProgramId;
    ...
    <c:if test="${remotePrograms!=null}">
        ...
        <display:table class="simple" cellspacing="2" cellpadding="3"
            ...
            <display:column sortable="true" title="Name">
                <a href="#javascript:void(0);"
                   onclick="selectRemoteProgram('<c:out
                           value="${program.facilityIdIntegerCompositePk.integratorFacilityId}"/>','<c:out
                           value="${program.facilityIdIntegerCompositePk.caisiItemId}"/>');"><c:out
                        value="${program.name}"/></a>
            </display:column>
            <display:column title="Facility Name">
                <c:set var="integratorFacilityId" value="${program.facilityIdIntegerCompositePk.integratorFacilityId}"
                       scope="request"/>
                <%
