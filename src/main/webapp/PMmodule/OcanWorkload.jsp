...
<html>
    ...
    <div id="demo">
        <table id="ocanTable" cellpadding="0" cellspacing="0" border="0" class="display" width="100%">
            ...
            %>
            <tr class="gradeB">
                <td style="text-align:right"><%=ocan.getAssessmentId() %>
                </td>
                <td style="text-align:right"><a
                        href="<%=request.getContextPath()%>/PMmodule/ClientManager.do?id=<%=ocan.getClientId()%>"><%=ocan.getClientId() %>
                </a></td>
                <td><%=ocan.getLastName() %>
                </td>
                <td><%=ocan.getFirstName() %>
                </td>
                <td style="text-align:center"><%=ocan.getAssessmentStatus() %>
                </td>
                <td style="text-align:center"><%=startDateStr %>
                </td>
                <td style="text-align:center"><%=completionDateStr %>
                </td>
                <td style="text-align:center"><%=convertOcanReasonUserString(ocan.getReasonForAssessment()) %>
                </td>
                <td style="text-align:center"><%=(ocanLead) ? "Yes" : "No" %>
                </td>
                <td style="text-align:center"><%=reassessmentTimeFrame %>
                </td>
                <td nowrap="nowrap">
                    <a href="javascript:void(0);"
                       onClick="beginReassign('<%=ocan.getAssessmentId()%>','<%=ocan.getClientId()%>')"><%=p.getFormattedName() %>
                    </a>
                </td>
            </tr>
            <%
    ...
    <div id="reassign_div" style="display:none">
        ...
        <form id="reassign_form" action="<%=request.getContextPath()%>/PMmodule/OcanWorkload.do" method="POST">
            ...
            <select name="reassign_new_provider" id="reassign_new_provider">
                ...
                %>
                <option value="<%=p.getProviderNo()%>" <%=(p.getProviderNo().equals(provider.getProviderNo())) ? " selected=\"selected\" " : "" %> ><%=p.getFormattedName()%>
                </option>
                <%
                    }
                %>
